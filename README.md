...
| `pptx` | 슬라이드 단위 청크. **그룹 도형(재귀)·표(셀 단위)·제목** 추출, 발표자 노트 옵션 (매뉴얼 탭) |
| `docx` / `pdf` | Docling으로 마크다운 변환(표는 TableFormer로 인식) → 헤더 기준 섹션화 (매뉴얼 탭, 프로덕션) |
...
> **PPT를 PDF로 변환해 넣는 경우**: PDF는 Docling 경로를 탄다. 텍스트 상자·표(TableFormer)는
> 잘 추출되지만, **① 발표자 노트, ② 슬라이드 번호(page_no), ③ SmartArt/다이어그램의 구조**는
> PDF 변환 과정에서 사라진다(텍스트만 남고 도형 관계는 유실). 원본 `.pptx`가 있으면 **PDF로 바꾸지
> 말고 pptx를 그대로 업로드**하는 편이 추출 품질이 더 좋다(네이티브 pptx 경로가 슬라이드 단위·표·
> 그룹도형·노트를 구조적으로 읽음). 스캔/이미지가 포함된 PDF는 `DOCLING_OCR=1`로 OCR을 켠다
> (폐쇄망은 OCR 모델도 사전 반입 필요).
