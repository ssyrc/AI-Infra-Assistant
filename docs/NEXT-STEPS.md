...
**이번 핵심 (#155 + #156 + #157)** — 아직 서버에 안 올라간 것까지 한 번에 반영됩니다.

0. **VOC 처리 주체를 LLM이 판정합니다** (#157). 물어보신 "user인지 operator인지 어떻게
   파악하느냐" — 지금까지는 답변에 `확인 결과`·`재기동` 같은 표현이 있는지 보는 키워드
   매칭뿐이었고, **매칭이 안 되면 전부 `user`로 떨어졌습니다**(주석에는 반대로 적혀 있었습니다).
   이제 "이 조치를 사용자가 자기 권한으로 재현할 수 있는가"를 LLM이 판정해 DB에 저장합니다.
   **반영 후 3번에서 분류를 한 번 돌려야 합니다.**
...
## 3. [서버] VOC 처리 주체 분류를 돌립니다 — **먼저 30건만**

판정 품질을 눈으로 보고 시작하세요. 이미 판정된 행은 건드리지 않으므로 여러 번 돌려도 됩니다.

```bash
cd /home/gpu1/yr9.choi/05_halo/AI-Infra-Assistant
bash scripts/classify-voc.sh 30
```

끝나면 표본 20건이 이렇게 나옵니다. **판정과 근거를 보세요.**

```
    id  판정        근거 / 문의
------------------------------------------------------------------
   142  operator   쿼터 증설은 운영자 권한이 필요
        └ 홈 디렉토리 용량이 부족합니다
   143  user       본인 제출 스크립트 수정으로 가능
        └ 작업이 메모리 부족으로 죽습니다
```

이상한 판정이 있으면 **그 id와 문의/답변을 보내주세요** — 판정 기준을 고칩니다.
괜찮으면 전체를 돌립니다(수천 건이면 몇 십 분 걸립니다. 끊어도 그때까지는 저장됩니다).

```bash
bash scripts/classify-voc.sh
```

콘솔 **VOC 탭**에도 같은 기능이 있습니다(`먼저 30건만` / `미분류 전부` 버튼 + 진행 표시).
목록에 `처리 주체` 열이 생겨서 판정과 근거를 행마다 볼 수 있습니다.

> 분류를 안 돌리면 예전 키워드 추론이 그대로 쓰입니다 — 동작은 하지만 운영자 건이
> `사용자`로 잘못 분류되어 **남이 해 준 조치를 사용자에게 시킬 수 있습니다.**

## 4. [서버] 선검색이 도는지 **먼저** 확인 — 여기서 갈립니다
...
**이게 안 나오면 아래 5번은 볼 필요 없습니다.** 원인이 여기입니다.

## 5. [웹] 확인 — Errors에 주신 시나리오 그대로
...
| 증상 | 조치 |
|---|---|
| 여전히 매뉴얼을 안 쓰는 것 같다 | **4번 로그 먼저**. `선검색` 줄이 없으면 코드가 안 올라간 것입니다 |
| 멀쩡한 질문에도 "운영팀에 문의" | 질문·답변·선검색 로그 세 개를 보내주세요 |
| 답변 끝에 `확인되지 않은 내용은 제외했습니다` | 지어낸 값이 든 줄을 뺀 것입니다. 그 답변을 보내주세요 |
| 답변이 한 번에 떠서 답답하다 | 6번. `answer_grounding_check` → `false` (지어내기 방지가 꺼집니다) |
| `ContextWindowExceededError` | `manual_prefetch_top_k`·`voc_prefetch_top_k` 를 `2`로 |
| VOC 판정이 이상하다 | 그 id와 문의/답변을 보내주세요(3번). 기준을 고칩니다 |
| 분류가 `실패`만 잔뜩 | 배치가 큰 것입니다. 설정 `voc_classify_batch`를 `6`으로 |
...
| 모델이 없는 옵션을 지어냄 | 7번에서 그 인자를 **선택형**으로 |
