...
**이번 핵심**: 지시문을 **절반으로 줄이고 라우팅 중심으로 다시 썼습니다**(#153).
`12,751자(24%)` → `5,687자(11%)`. 규칙은 하나도 빼지 않았고 중복·서술만 지웠습니다.

Errors에 주신 세 가지를 각각 고쳤습니다.

| 증상 | 조치 |
|---|---|
| 증상 문의("접속이 안 된다")에 job 목록을 실행 | **값**을 묻는가 / **왜·어떻게**를 묻는가로 가르고, "증상 호소는 상태 조회 요청이 아니다" 명시. "답과 상관없는 도구를 부르지 않습니다" |
| 앞 턴의 거절을 새 질문에 반복 | 처리 절차 **0단계**로 "이번 메시지 하나만으로 판단, 앞 턴의 거절을 이어붙이지 않는다" |
| 문서 안내를 코드블록으로 출력 | "코드 블록은 실행한 커맨드와 그 출력에만" |
...
## 3. [웹] 확인 — **VOC 문의를 그대로** 물어보세요

| 질문 | 기대 |
|---|---|
| VOC 엑셀의 실제 문의 (접속 안 됨 / timeout) | **매뉴얼·과거 사례로 답변.** job 목록 같은 무관한 실행이 없어야 합니다 |
| 그 문의를 **다른 질문 뒤에 이어서** | 앞 턴과 무관하게 **이 문의에 대한 답**이 나와야 합니다 |
| 문서 안내가 붙을 때 | `가이드 위치:` 가 **코드블록 밖**에 일반 텍스트로 |
| `내 홈 파일 리스트 보여줘` | 답변 + 원문 블록(전체 행) |
| `내 홈 디렉토리는 어디야?` | 실행해서 나온 값 |

무관한 실행이 또 보이면 **진행 줄을 통째로** 보내주세요 — 어떤 도구를 왜 불렀는지가 거기 있습니다.
