...
# manual_db v6/v7/v8, voc_db v6, command_db v6 적용 확인.
...
| "내 job 리스트 뭐가 있어?" | `phd info` 결과가 나온다. 실패하면 **10번**으로 |
| "gpu 사용법 알려줘" | 매뉴얼에 있는 내용은 컴파일 옵션까지 그대로 나와야 한다. 문서 안내는 `… > 활용 가이드 > GPU 서버 활용 가이드`처럼 **PPT 이름까지** 나와야 한다 |
| "슈퍼컴 계정 신청 어떻게 해?" | 1→2→3→4 단계가 **중간이 비지 않고** 이어진다 |

## 8. [웹] 설정 탭 — VOC 접수 경로 입력

`voc_intake_guide` 값을 **실제 사내 서비스 포탈 경로**로 바꾼다(기본값은 임시 문구).

## 9. [웹] 매뉴얼 탭 검색 테스트 — 7번이 여전히 못 찾을 때만
...
## 10. [서버] job/커맨드 실행이 여전히 ssh 인증 실패로 뜰 때
