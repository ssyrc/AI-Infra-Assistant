...
# platform_config v3(로그인 서버 IP 교체), manual_db v8, voc_db v7 적용 확인.
...
## 4. [웹] 매뉴얼 탭 — 활용가이드 CSV **다시 업로드** (`ppt_title` 열 매핑)

개별 PPT 이름을 청크마다 저장하게 바뀌었습니다. 기존에 올린 청크에는 그 값이 없으므로
**같은 CSV를 다시 업로드**해야 합니다(제목을 같게 하면 새 버전으로 올라갑니다).

열 선택 화면에 **② 문서명** 열이 새로 생겼습니다.
- `ppt_title` 열이 있으면 **자동으로 선택**됩니다. 표에서 ② 칸이 `ppt_title`에 찍혔는지만 확인하세요.
- 미리보기의 `문서명:` 줄에 실제 PPT 이름이 보이면 정상입니다.
- 등록 후 **발행**까지 하세요(발행해야 검색됩니다).

## 5. [웹] 매뉴얼 탭 — 문서 위치(참고 경로) 입력

문서를 클릭하면 상세 카드에 입력란이 있습니다. 말씀하신 대로 **활용가이드 메뉴까지만** 넣으세요.
개별 PPT 이름은 `ppt_title`에서 자동으로 뒤에 붙습니다.

```
슈퍼컴 Portal (https://supercom.samsung.net/sc/portal/main/main.do) > USEFUL INFO. > 활용 가이드 (User Guide)
```

→ 답변에는 `… > 활용 가이드 (User Guide) > GPU 서버 활용 가이드` 처럼 나갑니다.
"경로 저장"을 누르면 같은 제목의 모든 버전에 함께 적용됩니다.

## 6. [웹] 매뉴얼 탭 — 재임베딩

임베딩 입력에 문서 이름·섹션 제목을 붙이도록 바뀌었으므로(문맥 주입) **다시 임베딩해야**
적용됩니다. 매뉴얼 탭 상단 **"현재 설정으로 다시 임베딩"** 클릭.

## 7. [웹] Open WebUI `http://202.20.183.30:8502` — 지난번 5개 질문 그대로 확인

| 질문 | 통과 기준 |
|---|---|
| "내 홈 스토리지 용량 얼마 남았어?" | `myquota` 실행 결과 숫자가 나온다(20~60초 소요) |
| "내 홈 파일 리스트 보여줘" | 실제 파일 목록이 나오고, **계정명이 `{사용자 id}`로 가려지지 않고 그대로** 보인다 |
| "내 job 리스트 뭐가 있어?" | `phd info` 결과가 나온다. 실패하면 **11번**으로 |
| "gpu 사용법 알려줘" | 재컴파일은 **"CUDA 컴파일이 필요한 시뮬레이션 작업인 경우에만"** 처럼 조건이 붙어야 한다(무조건 필요한 것처럼 나오면 실패). 문서에 없는 `compute_90`을 지어내면 실패. 경로는 PPT 이름까지 |
| "슈퍼컴 계정 신청 어떻게 해?" | 1→2→3→4 단계가 **중간이 비지 않고** 이어진다 |

## 8. [웹] VOC 탭 — 업로드 묶음 / 선택 삭제

- 위쪽에 **업로드 묶음** 표가 생겼습니다. CSV 하나가 한 줄입니다.
  "이 묶음만 보기"로 걸러 보고, 잘못 올렸으면 **묶음 삭제**로 통째로 되돌립니다.
- 아래 목록은 **100건씩** 나옵니다(이전/다음). 맨 앞 체크박스로 골라 **선택 삭제**,
  헤더의 체크박스로 현재 페이지 전체 선택.
- 지금 올라가 있는 2천 건은 묶음 정보가 없어서(기능 추가 전 데이터) 묶음 표에는 안 나옵니다.
  목록에서 선택 삭제로 지우거나, 지운 뒤 다시 올리면 묶음으로 잡힙니다.
- **`.tsv`(탭 구분)도 올릴 수 있습니다.** 엑셀·CSV와 똑같이 헤더 행을 찾아 열을 고르는
  흐름입니다. 매뉴얼 탭·커맨드 탭도 같이 지원합니다.

## 9. [웹] 설정 탭 — VOC 접수 경로 입력

`voc_intake_guide` 값을 **실제 사내 서비스 포탈 경로**로 바꾼다(기본값은 임시 문구).

## 10. [웹] 매뉴얼 탭 검색 테스트 — 7번이 여전히 못 찾을 때만
...
## 11. [서버] job/커맨드 실행이 여전히 ssh 인증 실패로 뜰 때

```bash
cd /home/gpu1/yr9.choi/05_halo/AI-Infra-Assistant
bash scripts/diag-ssh.sh 202.20.185.100 yr9.choi myquota
```

맨 아래 "판정" 줄까지 그대로 전달해 주세요.
