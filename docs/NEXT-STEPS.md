...
## 3. [웹] 매뉴얼 탭 — 큰 파일 발행 다시

발행 버튼을 누르면 **"발행 중… (청크가 많으면 몇 분 걸립니다)"** 로 바뀝니다.
끝나면 `발행 완료 · 임베딩 N개` 가 뜹니다(임베딩 실패가 있으면 건수도 함께).

- 예전에는 청크를 하나씩 임베딩해서 수천 번 왕복했고, 화면에 아무 표시가 없어
  버튼이 안 눌리는 것처럼 보였습니다. 이제 32개씩 묶어 보냅니다.
- 중간에 끊겨도 이미 임베딩된 청크는 건너뛰므로 **다시 발행을 누르면 이어서** 진행됩니다.

## 4. [웹] 설정 탭 — 지시문 교체 (또 바뀌었습니다)
...
"내 홈 파일 리스트 보여줘" 를 묻고 **진행 상황 블록**을 보세요.

- `· 'ls -l' 실행하는 중` → `· 완료` 가 보이면 실제로 실행된 것입니다.
- **진행 상황에 실행 줄이 하나도 없는데 파일 목록이 나오면 = 지어낸 출력입니다.**
  (지난번 출력이 매번 똑같았던 게 그 증거입니다. 지시문에 금지 규칙을 넣었습니다.)
- 실행됐다면 계정명이 `yr9.choi` 그대로 나와야 합니다.

## 7. [웹] 매뉴얼 탭 검색 테스트 — `myquota` 가 왜 안 잡히는지

검색 테스트에 **"내 홈 스토리지 용량"** 을 넣고 결과 화면을 통째로 전달해 주세요.
`mode` / `embedding` / `published_chunks` / `min_score` / 각 결과 점수가 다 나옵니다.
이걸 봐야 매뉴얼에 없는 건지, 있는데 점수에서 걸린 건지 가릅니다.
(챗봇과 **같은 코드**를 쓰므로 여기 결과가 곧 챗봇 결과입니다.)

## 8. [웹] Open WebUI — "방화벽 관련 VOC" 다시

답변 끝에 **없는 가이드 문서를 지어내지 않아야** 합니다.
문서를 안내한다면 매뉴얼에 실제로 있는 PPT 이름 + 전체 경로로 나와야 합니다.
`{부문명}` 같은 자리표시자를 "DS부문"으로 되메우는 것도 실패입니다.

## 9. [웹] Open WebUI — GPU → CPU 순서로 다시

"gpu 사용법 알려줘" → "cpu 사용법 알려줘" 순으로 물어보세요.
CPU 답변에 GPU 내용이 섞이면 실패입니다. 그래도 섞이면 설정 탭
`rerank_min_score` 를 `0.05` → `0.2` 로 올려 보세요(관련도 낮은 문서를 더 세게 걸러냅니다).
