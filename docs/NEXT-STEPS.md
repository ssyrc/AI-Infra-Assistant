...
- **훨씬 빨라졌습니다.** 임베딩을 32건씩 묶어 보내서, 2천 행이면 요청이 2천 번 → 63번입니다.
- 등록 중에는 버튼 옆에 "등록 중입니다…" 안내가 뜹니다(예전엔 아무 표시가 없어 멈춘 것처럼 보였습니다).
- 긴 행 하나 때문에 전체가 죽지 않고, `field larger than field limit`도 해결됐습니다.
- 끝나면 `N건 등록 · M건 제외` 뒤에 **임베딩 실패 건수와 사유**가 뜹니다(0건이면 안 뜹니다).
  그 내용을 그대로 전달해 주세요.
- 부서·해결 여부·제외 기준 열은 화면에서 없앴습니다. 질문/답변 열만 고르면 됩니다.

---

## 부록. `agent_system_instruction` 전문 (설정이 날아갔을 때만)
