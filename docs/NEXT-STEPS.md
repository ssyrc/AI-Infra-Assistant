...
**이번 핵심**: 지어내기를 **코드로 잡습니다**(#154). 지시문으로 네 번 막았는데 네 번 재발해서,
#150 원문 블록과 같은 방식으로 **LLM을 거치지 않고** 경고를 붙입니다.

답변에 **조회 결과에 없는 IP·경로**가 있거나, **매뉴얼을 검색하지도 않고 문서를 안내**하면
이렇게 찍힙니다.

> ⚠️ **아래 내용은 근거가 확인되지 않았습니다 — 그대로 믿지 마세요.**
> 조회 결과에 없는 값: `10.20.30.40`

그리고 #153에서 제가 실수로 약하게 만든 부분을 되돌렸습니다 — 지시문을 줄이면서 "접속·계정·
스토리지·스케줄러·정책"이라는 **범위 정의**를 중복으로 보고 지웠는데, 그게 매뉴얼 검색을
발동시키던 것이었습니다. "이 회사 시스템에 관한 것이면 매뉴얼 검색은 선택이 아니다"로 되살렸습니다.
...
> ⚠️ 지난번 `길이는 같은데 내용이 다릅니다`도 **스크립트 버그**였습니다(#152).
> 구분자로 쓴 `\x01`이 psql에서는 4글자, 파이썬에서는 1바이트가 되어 **내용이 같아도 항상**
> 그 메시지가 나왔습니다. md5 비교로 고쳤고, **진짜 postgres를 띄워** 세 갈래 전부 확인했습니다.
...
```
코드 : 12751자  md5 eb6144e24860739e1021b1ec57f7b264
DB   : 12751자  md5 eb6144e24860739e1021b1ec57f7b264
...
## 3. [웹] **매뉴얼 문서마다 "위치"를 채워 주세요** — 이건 데이터 문제입니다

가이드 제목만 나오고 위치가 안 나온 것은 코드 문제가 아닙니다. 안내에 쓰는 `guide_location`은
**매뉴얼 등록 시 넣는 `문서 위치`(reference_path)** 에서 옵니다. 비어 있으면 지시문대로
제목만 씁니다.

콘솔 → **매뉴얼 탭** → 문서마다 **문서 위치**를 채우세요.
예: `슈퍼컴 Portal > 활용 가이드 > 접속 가이드` 또는 URL 전체.

채운 뒤 그 문서가 걸리는 질문을 하면 이렇게 나와야 합니다(코드블록 아님).

자세한 내용은 다음 문서를 참고하세요:
 - 가이드 위치: 슈퍼컴 Portal > 활용 가이드 > 접속 가이드
 - 가이드 문서: 슈퍼컴퓨팅센터 사용자 매뉴얼

## 4. [웹] 확인 — Errors에 주신 질문 그대로

| 질문 | 기대 |
|---|---|
| `~~~ 접속이 안되면 슈퍼컴 접속 못 하는거 아니야?` | **매뉴얼 검색 후** 그 내용으로 답변. 진행 줄에 매뉴얼 검색이 보여야 합니다 |
| `login server 접속이 갑자기 안됩니다…` | 매뉴얼·과거 사례 기반. **없는 IP를 만들면 ⚠ 경고가 붙습니다** |
| 다른 사용자 문의 (거절 대상) | 한 줄 거절. **가이드 안내가 붙으면 안 됩니다** |
| VOC 문의를 **다른 질문 뒤에 이어서** | 앞 턴과 무관하게 이 문의에 답해야 합니다 |

**⚠ 경고가 붙은 답변은 통째로 보내주세요.** 무엇을 지어냈는지 그 줄에 있습니다.
경고가 없는데도 틀린 안내가 나오면, 그건 검사가 못 잡는 종류이니 알려주세요.
...
## 5. [웹] 커맨드 인자를 **선택형**으로 — 엑셀에 드롭다운을 넣었습니다
...
| 모델이 없는 옵션을 지어냄 | 5번에서 그 인자를 **선택형**으로 바꾸세요 |
| ⚠ 경고가 너무 자주 붙는다 | 설정 탭 `answer_grounding_check` 로 끌 수 있습니다(권장하지 않음) |
