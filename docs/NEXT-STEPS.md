...
## 3. [웹] Open WebUI `http://202.20.183.30:8502` — yr9.choi 계정

- **"안녕"** — 먼저 이것부터. `Context variable not found: 사업부명` 오류가 사라지고
  평범한 인사 답변이 나와야 한다. 여기서 막히면 아래는 볼 필요 없다.
- "내 홈스토리지 용량 어떻게 돼?"
...
## 부록. `agent_system_instruction` 전문 (설정이 날아갔을 때만 붙여넣기)

이미 적용해 두셨습니다. 콘솔 설정이 초기화됐을 때 설정 탭에 이 전문을 붙여넣고 저장한 뒤
agent-server를 재시작하세요.
