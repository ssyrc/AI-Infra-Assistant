...
## 122. multi-turn에서 기억이 꼬이던 원인 — 장기기억이 절차를 외웠다 (완료)

Errors: "CPU 사용법, GPU 사용법, CPU에서 스크래치 사용법 물어본 다음에 GPU에서 스크래치
사용법 물어보면 CPU 용으로 알려주네."

**경로가 셋이었고, 셋 다 막아야 했다.**

### (1) 요약기가 인프라 절차를 장기기억으로 승격했다

`_summarize_turns`의 프롬프트가 "이 사용자에 대해 기억할 **사실/선호/맥락**"이었다. 기술
대화에서 그 지시를 주면 모델은 당연히 이런 걸 뽑는다.

> "사용자는 CPU 노드에서 스크래치를 /scratch/… 로 사용한다"

**절차는 기억할 것이 아니다** — 매뉴얼이 바뀌면 같이 바뀌어야 하고, 대상(CPU/GPU)마다 다르다.
프롬프트를 "이 사용자 **자신에 대한** 정보만(소속·담당·계정·서버·관심 주제·형식 선호)"으로
좁히고, **사용법·절차·명령어·경로·옵션·설정값은 절대 뽑지 말라**고 명시했다.

### (2) 그 장기기억이 시스템 지시문에서 근거처럼 쓰였다

`format_memory_block()`이 붙이던 헤더가 문제였다.

> `## 이 사용자에 대해 기억된 정보(참고용, 확실하지 않으면 근거를 다시 확인)`

"참고용"과 "확실하지 않으면"은 너무 약하다. **시스템 지시문에 실리는 글**이라 모델에게는
매뉴얼 검색 결과와 대등하거나 그 이상으로 보인다. 게다가 의미검색으로 뽑으니
"GPU 스크래치" 질문에는 "CPU 스크래치" 항목이 **반드시** 걸린다(임베딩이 거의 같다).

헤더를 바꿨다: **"근거가 아닙니다. 사용법·절차·경로·옵션·설정값을 여기서 가져오지 마세요.
아래 내용과 다르면 검색 결과가 맞습니다. 아래에 답이 있어 보여도 그것으로 답하지 않습니다."**

### (3) 대화 이력의 앞 답변 재사용

지시문에 "대상만 바뀐 질문일수록 위험합니다(GPU 다음의 CPU)"는 이미 있었지만, 이번 사례는
**주제(스크래치)가 같고 대상만 다른** 3턴 뒤의 질문이었다. 그 패턴을 실제 사례로 명시하고,
**그 대상의 내용이 검색에 안 나오면 앞 답변을 쓰지 말고 "확인되지 않습니다"라고 답하라**는
규칙을 넣었다.

### 이미 오염된 기억은 비웠다

`user_memory`에 이미 절차가 쌓여 있다. `source='auto-summary'`가 자동 요약분의 표식이라
그것만 지우는 마이그레이션(memory_db v2)을 넣었다. 사람이 직접 넣은 기억은 건드리지 않는다.
`conversation_state.summarized_upto`도 0으로 돌려 새 프롬프트로 다시 요약되게 했다.

**교훈**: "참고용"이라고 적어 두는 것으로는 시스템 지시문에 든 내용을 근거에서 배제할 수 없다.
애초에 **넣지 말아야 할 것을 넣지 않는 것**(요약 단계)이 먼저고, 문구는 두 번째 방어선이다.

## 123. 컨텍스트 초과(59,360토큰)의 진짜 원인 — 커맨드 출력에 상한이 없었다 (완료)

Errors: "gpu 인프라 현황 차트로 보여달라고 하면 ContextWindowExceededError...
your request has 59360 input tokens" + "차트가 제대로 작동하고 있지 않음"

**차트 문제가 아니었다.** 차트 툴 결과는 154자뿐이다(#114에서 그렇게 설계했다). 범인은
커맨드 출력이다.

```python
MAX_OUTPUT = 64 * 1024      # shared/ssh_exec.py
```

매뉴얼 검색 결과는 건당 1500자, VOC도 1500자로 자르고 있었는데 **커맨드 출력에만 상한이
없었다.** 64KB짜리 `nvidia-smi`/job 목록이 그대로 다음 요청 프롬프트에 실린다. "GPU 인프라
현황"이면 서버 여러 대를 돌기 때문에 두세 번만으로 59,360토큰이 된다.

#108에서 프롬프트 예산을 잴 때 **툴 스키마와 검색 결과만 봤고 커맨드 출력을 빼먹었다.**
실행 결과가 가장 클 수 있는 항목인데 세지 않았다.

### 조치

- `MAX_OUTPUT`을 64KB → **4,000자**로 낮추고 설정 키(`execution_result_max_chars`)로 뺐다.
- 자를 때 **줄 단위로** 자른다. 표 형태 출력을 줄 중간에서 끊으면 에이전트가 값을 잘못 읽는다.
  몇 줄이 빠졌는지 알려주고, **"여기 보이는 것만으로 답하고 전부라고 말하지 마세요"** 를 붙인다.
- `max_output` 기본값을 `None`으로 바꿨다. `def` 시점에 굳는 기본 인자였으면 설정을 바꿔도
  반영되지 않는다.
...
| 지시문 | 9,112 | 5,526 |
| 실행 툴 8개 | 2,160 | 762 |
| 매뉴얼 5×1,500 | 7,500 | 5,018 |
| VOC 5×1,500 | 7,500 | 5,018 |
| 대화 이력 | 8,000 | 5,352 |
| 커맨드 출력 4,000×3회 | 12,000 | 4,086 |
| **합계** | | **25,762** |

응답 생성용으로 ~7,000토큰이 남는다.

### 오류 문구도 고쳤다

litellm 스택트레이스를 그대로 내보내고 있었다. 사용자가 고칠 수 있는 문제인데 무엇을 해야
하는지 알 수 없다. 실제 수치와 조치를 담은 한국어 문구로 바꿨다.

```
한 번에 처리할 수 있는 양을 넘었습니다 (59,360 / 32,768 토큰).
출력이 많은 커맨드를 여러 번 실행했거나 대화가 길어진 경우입니다. 새 대화에서 다시
물어보시거나, 조회 범위를 좁혀 주세요(예: 서버 하나만, 기간을 줄여서).
```

다른 오류는 원문을 그대로 전한다(원인을 숨기면 안 된다).

## 124. 콘솔 설정 정리 — chart_public_base_url 숨김, Open WebUI 포트 설명 (완료)

**"아직도 관리자 콘솔에 chart_public_base_url 파라미터가 있던데 뭔지?"**
비워 두는 게 기본이고 권장인 값을 화면에 두면 "뭘 넣어야 하지?"가 계속 생긴다. 콘솔에서 뺐다.
코드 경로는 남겨 뒀다 — Open WebUI가 `data:` 이미지를 막는 환경에서만 `.env`나 DB로 켠다.

**"openwebui_base_url는 8080인데 실제 사용자가 접속하는건 8502"**
8080이 맞다. 그 값은 **관리자 콘솔이 도커 네트워크 안에서 Open WebUI API를 부를 때** 쓰는
주소이고, 8502는 그 8080에 매핑된 **호스트** 포트다. 내부망에는 8502가 없으므로 그걸 넣으면
동기화가 실패한다. 이미 자유 입력이라 컨테이너 이름·포트를 바꿨다면 고칠 수 있다.
혼동하지 않게 설명을 고쳤다: "관리자 콘솔이 Open WebUI API를 부를 도커 내부 주소.
사용자 접속 주소(8502)가 아니다".

## 125. 답변에 `(참고: …)` 꼬리말이 새고, `ls` 출력을 지어냈다 (완료)

Errors에 붙은 답변에 이런 줄들이 있었다.

```
(참고: 커맨드를 실행할 로그인 서버 주소는 '202.20.185.100'입니다.)
(참고: 운영팀 접수 경로는 'AI Infra 포탈 (https://…) → …'입니다.)
(참고: GPU_서버_활용_가이드_(KOR))
```

**앞 두 줄은 내가 지시문 끝에 붙인 것이다.** `agent.py`에서 로그인 서버 주소와 운영팀 접수
경로를 매 요청 주입하는데, 그걸 `(참고: …입니다.)` 형태의 **괄호 문장**으로 붙였다. 모델은
그걸 '답변 꼬리말 서식'으로 읽고 답변에 그대로 베꼈고, **같은 모양으로 세 번째 줄을 새로
만들어** 붙이기까지 했다(문서 이름을 그 서식에 끼워 넣었다).

시스템 지시문에 넣는 값은 **모양이 곧 지시**다. 문장처럼 쓰면 문장으로 흉내낸다.
라벨 블록으로 바꾸고 "이 블록을 답변에 옮기지 마라, `(참고: …)` 꼬리말을 만들지 마라"를 넣었다.

```
# 이 환경의 값
아래는 도구 호출과 안내에 쓰는 값입니다. **이 블록을 답변에 옮겨 적지 마세요.**
- 로그인 서버 주소: 202.20.185.100
- 운영팀 접수 경로: …
```
...
```
drwxr-xr-x 2 ops_assistant users 4.0K 10월 15 10:23 Desktop
```

`ops_assistant`는 **에이전트 자기 이름**이다(`agent.py`의 `APP_NAME`/`name`). ADK가 시스템
프롬프트에 에이전트 이름을 넣으므로 모델이 그걸 보고 사용자 계정처럼 썼다. 목록도
Desktop/Documents/Downloads… 전형적인 데스크톱 홈이다 — 로그인 서버의 실제 홈이 아니다.
**즉 도구를 부르지 않고 만들어 낸 출력이다.**

왜 도구를 안 불렀는지는 아직 못 짚었다. 후보는 `list_dir`이 꺼져 있어서(#117에서 비활성 툴을
목록에서 빼도록 바꿨으므로 꺼져 있으면 아예 안 보인다) 쓸 도구가 없었을 가능성이다.
그래서 기동 로그에 **노출된 툴 이름을 전부 찍게** 했다 — 목록을 보면 바로 확인된다.

지시문에는 이 사고를 그대로 넣었다: 자기 이름이나 흔한 예시를 사용자 계정·파일처럼 쓰지 말고,
쓸 도구가 없으면 **"실행할 수 있는 도구가 없어 확인하지 못했습니다"** 라고 답하라고.

## 126. 인프라 현황 차트가 컨텍스트를 넘긴 이유 (완료)

33,413 / 32,768. #123에서 커맨드 출력을 4,000자로 잘랐는데도 넘쳤다.

에이전트가 "GPU 상태 확인하는 중 → 저장공간 확인하는 중"으로 **서버마다 커맨드를 돌렸다.**
그런데 사용자 말대로 그 정보는 **매뉴얼에 올려둔 것**이다. 커맨드를 돌릴 필요가 없었다.

지시문에 넣었다.
- "GPU 인프라 현황", "서버 구성", "노드 스펙"처럼 **정해져 있는 정보는 매뉴얼을 먼저 검색**한다.
  실시간 상태(지금 사용률, 지금 빈 노드)를 물었을 때만 커맨드를 쓴다.
- 커맨드를 여러 서버에 반복하면 출력이 쌓여 한도를 넘는다. 꼭 필요한 것만 한 번씩.
- **도구를 이어서 쓴다**: 매뉴얼 검색으로 수치를 얻고 → 그 수치로 차트 도구를 호출 →
  표/그림으로 답한다. 수치를 못 얻었으면 차트를 만들지 않는다.

오류 문구에서 설정 키 이름도 뺐다(`execution_result_max_chars` 같은 건 관리자만 볼 값이다).
"운영팀에 알려주세요"로 끝낸다.

## 127. ssh 세션을 더 오래 살려 둔다 (완료)

"채팅 시작하자마자 ssh 세션이 시작되어야 해."

마스터는 MCP 기동 시 예열하고 주기적으로 유지한다. 다만 `ControlPersist`가 **300초**여서
5분 넘게 쉬었다 물으면 재접속해 1~3초가 붙었다. 채팅이 드문드문 오는 사용 패턴이라
**3600초**로 늘리고, 유지 주기도 240초 → 180초로 줄였다. 유휴 ssh 연결 하나의 비용은
무시할 만하다.

기동 로그에 마스터 상태가 찍히므로(#120) "느리다"가 이 문제인지 바로 갈린다.

## 128. "job 리스트가 예전엔 바로 나왔는데 느리다" — 재지 않고 고치던 것을 멈췄다 (완료)

Errors: *"execution_mcp 성능이 별로야. 예전에는 gpu job 리스트 보여줘 하면 바로 보여줬었는데
지금은 안 그래."* + 콘솔 정리 요청 몇 가지.

#120에서 같은 리포트를 받고 마스터 연결 상태를 로그에 찍게 했지만, **한 커맨드가 몇 초
걸리는지는 여전히 아무 데도 남지 않았다.** 그래서 이번엔 재는 것부터 했다.

### 먼저 — 어디서 시간을 쓰는지 보이게 만들었다

한 번의 실행은 세 덩어리다: (1) ssh 접속, (2) 원격 `su -` 로그인 셸, (3) 커맨드 자체.
그리고 요청 전체로 보면 여기에 **LLM 턴 수 × 프리필**이 곱해진다. 어느 쪽인지 모르면
또 엉뚱한 데를 고친다(#69에서 빈 키 파일·호스트 키·TTY를 차례로 의심했다가 전부 틀렸다).

- `run_ssh_as_user`가 `duration_ms`와 `connection_reused`를 결과에 담고 로그에도 찍는다.
  재사용 여부를 알려면 소켓 경로를 우리가 알아야 하는데, ssh의 `%C`는 파이썬에서 계산할 수
  없다 → **`control_path(ip)`로 우리가 정하고** `os.path.exists()`로 확인한다.
- 진행 상황 줄에 초가 보인다: `· 완료 (202.20.185.100 · yr9.choi · 2.3초)`.
  사용자가 채팅 화면에서 바로 "이 커맨드가 느리다"를 짚어 줄 수 있다.
- agent-server가 요청마다 한 줄 남긴다:
  `[agent] chatcmpl-… 완료 12.3초 (첫 글자 4.1초 · 도구 2회 · 커맨드 실행 3.5초 · yr9.choi)`.
  **전체 − 커맨드 = LLM이 생각한 시간**이다. 이 한 줄로 "실행이 느린 것"과 "턴이 많은 것"이 갈린다.
- `scripts/bench-exec.sh` 추가: 접속만 / `su -` 로그인 셸만 / 실제 커맨드를 따로 재서
  (1)(2)(3)을 분해한다. HPC 로그인 노드는 프로필이 무거워 (2)만 수 초인 경우가 흔한데,
  그건 다중화로 줄지 않고 **도구 호출 횟수만큼 곱해진다**.

### 그다음 — 실제로 줄인 것

**(1) compose가 코드 기본값을 덮어쓰고 있었다.** #127에서 `ControlPersist`를 300 → 3600으로
올렸는데, `docker-compose.dev.yml`에 `SSH_CONTROL_PERSIST: ${…:-300}`이 그대로 박혀 있었다.
**환경변수가 이기므로 그 변경은 아무 효과가 없었다.** 5분만 쉬어도 마스터가 죽어 다음 질문의
첫 커맨드에 1~3초가 다시 붙는다. 3600으로 맞추고, 유휴 연결을 끊는 방화벽 대비로
`ServerAliveInterval`도 넣었다. 운영 compose에는 다중화 설정 자체가 없어서 함께 넣었다.

**(2) 툴 호출마다 DB를 세 번 왕복했다.** `enabled` 한 번, `required_roles` 한 번(같은 행이다),
성공 감사로그 INSERT 한 번. 앞의 둘을 한 쿼리로 합치고, 성공 로그는 결과를 이미 손에 쥔 뒤라
**응답을 막지 않고 뒤에서 쓴다**(`asyncio.create_task`는 컨텍스트를 복사하므로 호출자 신원도
그대로 남는다). 거부/차단은 실행되지 않아 빠르므로 그대로 동기로 둔다.

**(3) 대화 이력 주입이 턴 수의 제곱이었다.** `_create_session`이 이력 한 턴마다
`get_session()`을 다시 불렀는데, 그건 그때까지 쌓인 이벤트를 **전부 다시 읽어 온다**.
10턴이면 왕복 20번 + 매번 커지는 페이로드다. 세션 객체를 한 번만 읽게 고쳤다
(`append_event`가 메모리의 세션에도 이벤트를 더해 준다). 이건 사용자가 첫 글자를 보기 전의 지연이다.

**(4) "job 현황"이 매뉴얼 검색을 먼저 태우고 있었다.** #126에서 넣은 규칙("인프라 현황·구성은
매뉴얼을 먼저")이 **'현황'이라는 낱말만 보고** 본인 자원 질문까지 잡아갔다. 매뉴얼 검색 한 번은
임베딩 + 3축 검색 + 리랭킹이고, 그 앞뒤로 LLM 턴이 하나 더 붙는다. 지시문에 예외를 명시했다 —
"내 job 현황", "내 작업 현황"처럼 본인 자원은 매뉴얼에 있을 수 없는 실시간 값이니 바로 실행한다.
"확인해 달라" 절에도 **"맞는 툴 하나를 골라 한 번에 실행하고 끝낸다, 매뉴얼도 과거 사례도 먼저
뒤지지 않는다"**를 넣었다.

### 코드 내장 커맨드 7개 삭제 (사용자 요청, 겸사겸사 프롬프트도 가벼워졌다)

*"모든 내장 7개 코드는 삭제해. LLM이 아는 커맨드에 있다는 거잖아."* — 맞는 말이다.
`gpu_status`(nvidia-smi), `list_dir`(ls), `disk_free`(df), `disk_usage`(du), `find_files`(find),
`read_file_head`(head), `system_info`(uptime 등)는 전부 LLM이 이미 아는 표준 명령이고
`run_command`로 그대로 실행된다(차단 목록에 없다).

그런데 이것들은 **편집도 삭제도 안 되면서** 설명·스키마가 매 요청 프롬프트에 실렸다.
없애니 툴 목록이 그만큼 줄고, "이건 왜 못 고치지?"도 없어졌다.

- `builtin.py`·`linux_exec.py` 삭제, `execution_builtin_state`를 읽는 코드 전부 제거
  (테이블은 되돌릴 수 있게 남긴다). 콘솔 API `/builtin` 두 개도 삭제.
- 잃은 것: `lines` 1~2000, `max_depth` 0~10, `safe_path()` 같은 **인자 값 검증**.
  남는 방어선은 차단 목록(모든 토큰 엄격 검사) + 항상 본인 권한 강등 + 셸 미사용 argv다.
  값 검증이 꼭 필요한 커맨드는 콘솔에 자리표시자와 타입을 붙여 등록하면 된다.
- 부수 효과: 콘솔 이미지가 더 이상 `mcp_servers`를 복사하지 않는다. 예전에 그 파일 하나를
  COPY하다 경로가 바뀌어 운영 빌드가 깨진 적이 있다(#112) — 그 구멍이 원천적으로 없어졌다.

### 콘솔 정리 (Errors 요청 그대로)

- 커맨드 표에서 **'인자' 열 제거**("코드에서 검증 / 없음+자유 인자"는 볼 이유가 없다).
- 이름 아래 `disk_free · 코드 내장` **중복 표기 제거**(내장이 없어져 자연히 사라졌다).
- 등록 폼의 **'추가 인자 허용' 체크박스 제거 → 항상 허용.** 어떤 인자가 필요한지는 질문마다
  달라 관리자가 미리 정할 수 없고, 위험한 인자는 차단 목록이 어차피 막는다.
- 이제 **모든 행이 똑같이** 편집·삭제·활성 토글된다.

### `scheduler_login_host` → `execution_host` 개명

*"scheduler_login_host로 지정되어 있어서 그런가? 그냥 execution_host라고 바꾸고…"* —
'scheduler'라는 이름 탓에 스케줄러 전용 설정으로 읽혔는데, 실제로는 **모든 커맨드가 실행되는
서버 주소**다. platform_config v6 마이그레이션이 값을 그대로 옮기고 옛 키를 지운다
(개명 때문에 ssh가 끊기면 안 된다). `.env`/compose의 `SCHEDULER_LOGIN_HOST`도
`EXECUTION_HOST`로 바꿨고, `.env.example`에 남아 있던 `login05`(이름!)를 IP로 고쳤다.

### 그 외 Errors 항목

- **Open WebUI 사용자 접속 주소 설정 추가**: `openwebui_public_url`(기본 `…:8502`).
  기존 `openwebui_base_url`(도커 내부 8080)은 콘솔이 API를 부를 때만 쓰는 값이라 그대로 둔다.
- **컨텍스트 초과 안내에 접수 경로**: "운영팀에 알려주세요"로 끝내면 어디로 알릴지가 빠진다.
  콘솔의 `voc_intake_guide` 값을 그대로 붙인다.
- `manual_mcp_url`/`execution_mcp_url` 등이 하는 일: **agent-server가 그 주소로 MCP에 붙는다.**
  값은 도커 내부망 주소이고, 바꾼 뒤 agent-server를 재시작하면 그때부터 새 주소를 본다
  (`hot_reload=false`라 설정 탭 그룹마다 재시작 버튼이 붙어 있다).

### 검증

회귀 테스트 64개 통과. 새로 고정한 것 4개:
계측이 결과·진행 줄·요청 로그에 실제로 실려 있는지, 툴 호출당 DB 왕복이 한 번이고 성공 로그가
응답을 막지 않는지, 이력 주입이 세션을 다시 읽지 않는지, 지시문이 본인 자원 질문을 바로
실행으로 보내는지. 내장 커맨드를 보던 테스트 4개는 "코드에 커맨드를 다시 박으면 실패하는"
테스트로 바꿔 놓았다.

**아직 안 잰 것.** 위 조치로도 느리면 남은 후보는 둘이다 — (a) 원격 `su -` 로그인 셸이 무겁다
(`bench-exec.sh`의 2번 항목), (b) LLM 프리필이 길다(지시문 ~5k토큰 + 툴 스키마).
둘 다 `[agent] … 완료` 로그 한 줄로 갈리므로, 다음 라운드에는 추측하지 않는다.

## 129. `phd info -u {user_id}`를 계속 쓰던 이유 — **내가 지시문에 적어 놨다** (완료)

Errors: *"도대체 phd info -u {user_id} 는 왜 쓰는거야? 이거 안 되는 커맨드라고."*

찾아보니 원인이 명확했다. `shared/migrations.py`의 `AGENT_INSTRUCTION`에 이 줄이 있었다.

```
`phd info` 같은 사내 커맨드를 기억으로 지어내지 말고, **툴 목록에 있는 것**을 씁니다.
```

**금지 예시로 쓴 건데, 지시문은 매 요청 시스템 프롬프트다.** 모델 입장에서 거기 적힌
문자열은 '쓰면 안 되는 것'이 아니라 '이 시스템에 있는 커맨드 이름'으로 읽힌다.
그래서 툴 목록에도 매뉴얼에도 없는 그 이름을 `run_command`에 그대로 넣고 있었다.

**같은 실수를 세 번째 한 것이다.**
- #74: "컴파일 옵션을 붙이지 마라"는 금지 목록이 문서에 **있는** 내용까지 억눌렀다.
- #125: 환경 값을 `(참고: …)` 괄호 문장으로 붙였더니 그 서식을 답변에 베꼈다.
- 이번: 금지 예시로 든 커맨드 이름을 그대로 실행했다.

세 번 다 **"지시문에 쓴 것은 모양이든 이름이든 모델이 따라 한다"**는 같은 성질이다.
지시문에 넣은 구체적인 문자열은 예시가 아니라 **재료**로 취급된다.

### 조치

- 지시문에서 커맨드 이름을 **완전히 뺐다.** 대신 규칙만 남겼다: "사내 전용 커맨드의 이름을
  기억에서 꺼내 쓰지 않는다. 스케줄러·큐·쿼터 조회처럼 회사마다 다른 커맨드는 이 시스템에서
  무엇인지 알 수 있는 곳이 **툴 목록과 매뉴얼뿐**이고, 거기 없는 이름을 넣으면
  `command not found`가 날 뿐이다."
  (`ls`·`df`·`find` 같은 **표준 리눅스 명령은 그대로 둔다** — 실제로 존재하므로 예외다.)
- 저장소 전체에서 그 이름을 걷어냈다(코드 주석·콘솔 docstring·테스트 픽스처·bench 스크립트
  예시). 문자열이 남아 있으면 다음에 또 문서·프롬프트로 새어 들어간다. 두 토큰 커맨드가
  필요한 자리에는 명백한 예시(`quota report`)나 실제로 동작이 확인된 `myquota`를 쓴다.
- 회귀 테스트 `test_instruction_names_no_in_house_command` 추가 — 지시문에 사내 커맨드
  이름이 들어가면 실패한다(금지 예시로 쓰는 것도 막는다).

### 남은 가능성 — 콘솔에 등록돼 있을 수 있다

지시문 말고 경로가 하나 더 있다. **실행 탭에 `phd info -u {user_id}`가 등록돼 있으면**
그건 그대로 에이전트 툴이 되므로 계속 호출된다(#98에서 "이제 실행은 되네"라고 한 게 바로
그 등록분이다). 이건 코드가 아니라 데이터라 우리가 지울 수 없다 —
NEXT-STEPS 3번에 콘솔에서 확인/삭제하도록 넣었고, 기동 로그의 `노출된 툴:` 줄로 바로 확인된다.

**교훈.** 지시문에 구체적인 이름·서식·값을 쓸 때는 "이걸 모델이 그대로 쓰면 어떻게 되나"를
먼저 본다. 금지하려면 이름을 적지 말고 **규칙과 출처만** 적는다.

## 130. 측정값이 나왔다 — 범인은 **첫 접속 17.4초**(인증 협상) (조치)

`bench-exec.sh` 실측(사용자 제공):
...
=== 2회차 이후: 마스터 재사용 ===
  접속만 (true)           1.11초
  su - 로그인 셸만        3.10초
  실제 커맨드(phd list)   7.12초
```

**읽으면 이렇게 갈린다.** 2회차 "접속만" 1.11초에는 단계마다 새로 띄운
`docker compose exec` 기동 비용(약 1초)이 섞여 있다 — 내 계측 스크립트의 결함이다.
그걸 빼면 마스터가 있을 때 접속은 사실상 0이고, 나머지는
**`su -` 로그인 셸 약 2초 + 커맨드 자체 약 4초**다.

그리고 **마스터가 없으면 첫 커맨드에 17.4초**가 붙는다. 이게 "예전엔 바로 나왔는데"의 실체다.
17초는 TCP 연결 시간이 아니다(그건 `ConnectTimeout=8`에 걸린다). **인증 협상 단계**다.

### 조치 1 — 핸드셰이크를 늘리는 것들을 껐다

- `GSSAPIAuthentication=no` — 커버로스 KDC가 없거나 안 닿는 망에서 조회가 타임아웃까지
  매달린다. 폐쇄망에서 수 초~수십 초가 흔하고, 우리는 공개키만 쓴다. **가장 유력한 범인.**
- `IdentitiesOnly=yes` — 지정한 키 하나만 시도한다(다른 키를 던져 보는 왕복 제거).
- `PreferredAuthentications=publickey` — 다른 방식 협상을 아예 건너뛴다.
- `AddressFamily=inet` — AAAA 시도 후 폴백하는 시간을 없앤다.

전부 클라이언트 옵션이라 게이트 서버를 건드리지 않는다. 서버 쪽 원인(역방향 DNS `UseDNS` 등)이면
이걸로도 안 줄어드는데, 그건 아래 조치 2로 감춘다.

### 조치 2 — "새 채팅 열면 세션이 서 있어야 한다"를 실제로 구현했다

Errors: *"사용자가 (1) 새로고침하거나 (2) 로그인/접속하거나 (3) 새 채팅을 열면 바로
202.20.185.100 서버로 ssh 세션이 시작되어야 해."*

지금까지는 **MCP 기동 시 1회 + 180초 주기 예열**뿐이었다. 마스터가 어떤 이유로 죽으면
다음 질문이 17초를 그대로 문다. 요청 시점에 여는 경로를 넣었다.

- Execution MCP에 `GET /warm` 라우트 추가. 이미 서 있으면 `already_warm`으로 즉시 반환하고,
  없으면 그 자리에서 마스터를 연다. **MCP 툴이 아니라 평범한 HTTP 라우트라 프롬프트에 안 실린다.**
- agent-server가 `/v1/models`(Open WebUI가 페이지를 열거나 새로고침할 때 부른다), `/health`,
  그리고 **매 채팅 요청 시작 시** 이 주소를 두드린다. `create_task`라 응답을 막지 않는다.
  사용자가 질문을 타이핑하는 동안 세션이 준비된다.

### 조치 3 — `su -` 2초를 끌 수 있게 했다(기본은 그대로)

로그인 셸은 커맨드마다 원격 계정 프로필을 전부 읽는다(실측 약 2초). 다중화로 줄지 않고
**도구 호출 횟수만큼 곱해진다.** 다만 사내 커맨드가 프로필의 PATH·모듈에 의존하면 끄는 순간
`command not found`가 되므로 **기본은 로그인 셸을 유지**하고, `SSH_SU_LOGIN=false`로만 끈다.
권한 강등(`su <user>`)은 어느 쪽이든 동일해서 우회 경로가 생기지 않는다.
켜기 전에 손으로 확인하는 커맨드를 NEXT-STEPS 5번에 넣었다.

### 계측 스크립트를 고쳤다

단계마다 `docker compose exec`를 새로 띄우던 것을 **컨테이너 안에서 한 번에** 돌도록 바꿨다.
그리고 1번 항목에서 **최적화를 끈 조건과 켠 조건을 나란히** 잰다 — 인증 협상이 원인인지
서버 쪽 지연인지 한 번에 갈린다.

**교훈.** `ConnectTimeout`은 TCP 연결에만 걸린다. "접속이 느리다"를 그 값으로 진단하면 안 된다.
그리고 계측 도구 자체의 오버헤드를 빼지 않으면(여기선 docker exec 1초) 작은 숫자를 오독한다.

## 131. 실서버 로그 3건 — 마스터 미유지 / 툴 인자 JSON 깨짐 / 자기 이름을 홈 경로로 (조치)

```
[ssh_exec] phd 25,048ms (새 접속 · 202.20.185.100 · yr9.choi)
[오류가 발생했습니다: Expecting value: line 1 column 11 (char 10)]
· 'ls' 실행하는 중 → 확인 완료
/home/ops_assistant 경로가 존재하지 않습니다.
```

### (1) `새 접속` — 예열이 안 붙어 있었다

25초짜리 실행이 **`새 접속`**으로 찍혔다. 계측을 넣은 보람이 바로 여기 있다 — 이 한 단어로
"마스터가 없었다"가 확정된다. #130의 조치(핸드셰이크 옵션 + 요청 시점 `/warm`)가 아직
배포 전이라 그대로 재현된 것이다. 반영 후 같은 줄이 `연결 재사용`으로 바뀌어야 한다.

### (2) `Expecting value: line 1 column 11 (char 10)` — 어디서 났는지 알 수 없었다

JSON 파싱 실패다. 그런데 **스트리밍 오류 로그가 메시지만 찍고 스택트레이스를 안 남겼다.**
그래서 툴 인자 파싱인지 응답 파싱인지 구분할 수 없었다 — 로그가 진단을 막고 있었다.
`traceback.format_exc()`를 남기도록 고쳤다.

가장 유력한 원인은 vLLM tool-call 파서(hermes)가 스트리밍 중 조각난 JSON을 내보내는 것이다.
사용자가 고칠 수 있는 게 없고 대개 재시도로 풀리므로, 원문 대신
"도구를 호출하는 형식이 깨져서 …. 같은 질문을 한 번 더 보내주세요"로 바꿨다.
다음에 나면 트레이스로 발생 지점을 특정한다.

### (3) `/home/ops_assistant` — 에이전트가 **자기 이름**을 계정으로 썼다

`ops_assistant`는 ADK가 시스템 프롬프트에 넣는 **에이전트 자신의 이름**이다.
#125에서 지어낸 `ls` 출력의 소유자로 쓴 것과 같은 뿌리인데, 이번엔 한 걸음 더 나가
그 이름으로 **경로를 조립해 실제로 실행**했다. 그러고는 "경로가 존재하지 않습니다"라고 답했다.

지시문으로 두 번 막았는데 재발했다. **지시문만으로는 못 막는다는 뜻이므로 실행 단계에서 끊는다.**
- `reject_self_name()` — argv 어디에든 에이전트 이름이 들어 있으면 거부한다(등록/미등록 양쪽).
  거부 메시지가 **다음 행동**을 알려 준다: "그건 사용자 계정이 아니다, 홈을 보려면 경로를
  비우거나 `.`을 써라, 실행은 항상 본인 홈에서 시작한다."
- 지시문과 `run_command` 설명에도 "`/home/...` 경로를 조립하지 않는다, 본인 홈은 경로를
  주지 않는 것이 정답"을 넣었다. 툴 설명은 짧아서 비용이 거의 없고 호출 직전에 읽힌다.
- `/home/gpu1/yr9.choi` 같은 정상 경로는 그대로 통과한다(회귀 테스트로 고정).

**진행 줄이 `확인 완료`로 나온 것도 고쳤다.** 실행 툴인데 exit_code를 못 찾으면 성공·실패가
구분되지 않는다. MCP 응답이 한두 겹 감싸여 오거나 JSON 문자열로 오는 경우까지 벗겨 내
`완료 (IP · 계정 · 초)` / `실패(종료코드 N)`가 제대로 찍히게 했다.

**교훈.** 같은 환각이 세 번 반복되면 그건 프롬프트로 고칠 문제가 아니다.
**실행 경로에서 거부하고, 거부 메시지에 올바른 방법을 적는다.**

## 132. 로그인 서버 root 세션을 **상주**시킨다 (완료)

요청: *"애초에 agent 서비스 시작할 때, 계속 202.20.185.100에 root로 세션 떠있게 해줘.
바로바로 agent가 su 명령어 날릴 수 있도록."*

맞는 요구다. 지금까지는 그 비슷한 것을 하고 있었지만 **연결의 수명을 우리가 쥐고 있지 않았다.**

| | 예전 | 지금 |
|---|---|---|
| 여는 방법 | `ssh … true` 한 번 실행(연결만 만들어 놓음) | `ssh -M -N` 을 **우리 자식 프로세스로** 붙들고 있음 |
| 수명 | `ControlPersist`(3600초)에 맡김 | 프로세스가 살아 있는 동안 |
| 죽으면 | 다음 확인(180초)까지 구멍 | **15초 안에 다시 띄움** |
| 살아 있는지 | 알 수 없음 | 프로세스 상태 + 소켓 파일 확인 |

구멍이 실제로 났다. 실서버 로그에 `[ssh_exec] phd 25,048ms (**새 접속** …)` 이 찍혔다 —
마스터가 없어서 커맨드가 직접 접속했고, 인증 협상에만 25초를 썼다.

### 구현

- `ensure_master(host)` — 살아 있으면 그대로 두고(우리가 띄운 게 아니어도 `-O check`로
  확인되면 **그대로 채택**한다), 없으면 `ssh -M -N`을 띄우고 소켓이 설 때까지 기다린다.
  결과 dict를 그대로 `/warm` 응답과 로그에 쓴다.
- `start_master_supervisor(..., interval=15)` — 15초마다 확인. 살아 있으면 소켓 파일 확인
  한 번으로 끝나 사실상 공짜다. 죽어 있으면 즉시 재시도하고, 연속 실패 시 간격을 늘린다
  (최대 5분) — 안 되는 대상을 계속 두드리지 않는다.
- `stop_masters()` — 서비스 종료 시 정리(shutdown 이벤트).
- 마스터 전용 옵션을 분리했다: `ControlMaster=yes` + **`ControlPersist=no`**.
  ControlPersist를 주면 ssh가 스스로 백그라운드로 넘어가 자식 프로세스가 곧바로 끝나 버려서
  **감시가 불가능해진다.** 전경에 붙들어 둬야 살아 있는지 알 수 있다.
  ControlPath는 커맨드 실행 쪽과 **반드시 같아야** 한다(다르면 재사용이 안 된다).
- 커맨드 실행 쪽은 그대로 `ControlMaster=auto`다. 마스터가 없어도 스스로 접속해서 동작은
  한다 — 느려질 뿐 끊기지 않는다. `SSH_CONTROL_PERSIST`는 이제 그 폴백 연결의 수명이다.

### 검증

실제 sshd 없이도 상태 기계를 돌릴 수 있게 **가짜 ssh**(소켓 파일 생성 + `-N`이면 상주 +
`-O check` 응답)를 만들어 회귀 테스트로 고정했다: 띄운다 → 이미 있으면 그대로 쓴다 →
죽이면 감시 루프가 복구한다. 셋 중 하나만 깨져도 커맨드마다 수십 초를 물게 되는 자리다.

### 짚어 둘 것 — 상주하는 것은 **root 전송 연결**이다

사용자별 로그인 셸을 띄워 두는 것이 아니다. `su - <user_id>`는 커맨드마다 실행된다(#103에서
...
## 134. "사용자별 셸을 하나 띄워 두면 안 되나" — 강등 방식을 고를 수 있게 했다 (완료)

질문 두 개.
1. *"우리 agent는 사용자 요청이 있을 때마다 `su - user.id`로 셸을 새로 띄우는 거야?"* → **그렇다.**
2. *"애초에 로그인 서버에 ssh 셸을 하나 만들고 시작하면 안 되나?"* → 전송 연결은 이미 그렇게
   하고 있다(#132). 하지만 **사용자 셸**을 상주시키는 것은 다른 이야기다.

### 지금 무엇이 상주고 무엇이 매번인가

```
[상주]  ssh root@202.20.185.100        서비스 기동 시 1개, 15초마다 감시 (#132)
[매번]  └─ su - <user_id> -c '<커맨드>'  커맨드 하나마다 (프로필 읽기 약 2초)
```

접속 비용은 이미 0이다. 남은 것은 **강등 + 로그인 셸** 비용이고, 그게 커맨드 수만큼 곱해진다.

### 사용자 셸을 상주시키지 않는 이유 (#103 재확인)

상주 셸에 커맨드를 넣으려면 **파이프에 문자열을 써 넣어야** 한다. 그 순간
CLAUDE.md §3의 "셸을 쓰지 않는다(argv 리스트로 실행)"가 깨지고, 지금까지 구조로 막아 온
셸 주입 표면이 그대로 열린다. 그 밖에도 출력 경계(어디까지가 이 커맨드의 결과인지),
요청 간 상태 누수(cwd·환경변수), 사용자 수만큼 늘어나는 유휴 셸, 멈춘 커맨드가 그 사용자의
셸을 영구히 막는 문제가 새로 생긴다. **얻는 것은 프로필 읽기 2초뿐인데 대가가 크다.**

### 대신 한 것 — 강등 방식을 고르게 하고, 셋을 나란히 재게 했다

비교 대상(spagent)이 빠른 진짜 이유가 여기 있다. 그쪽은 `paramiko.exec_command`라
**로그인 셸을 아예 거치지 않는다.** 같은 효과를 우리 안전 모델을 지키면서 낼 수 있다.

| `SSH_PRIVDROP` | 원격에서 실행되는 것 | 비용 | 인용 겹수 |
|---|---|---|---|
| `su-login`(기본) | `su - <user> -c '...'` | 프로필 매번 읽음(실측 ~2초) | 2겹 |
| `su` | `su <user> -c '...'` | 프로필 안 읽음 | 2겹 |
| `runuser` | `runuser -u <user> -- <argv>` | **PAM 인증·프로필 모두 건너뜀** | **1겹** |

`runuser`는 root 전용 도구라 인증 단계가 없고, **argv를 그대로 받는다.** 그래서 빠를 뿐 아니라
`su -c '문자열'`의 두 겹 인용이 한 겹으로 줄어 **안전 면에서도 낫다.** 세 방식 모두
호출자 본인 계정으로 내려가는 것은 동일하다 — 우회 경로가 생기지 않는다.

- 기본은 `su-login`을 **유지**한다. 사내 커맨드가 프로필에서 잡히는 PATH·모듈에 의존하면
  나머지 둘은 `command not found`가 된다. 추측으로 바꿀 수 있는 값이 아니다.
- `scripts/bench-exec.sh`가 셋의 **고정 비용과 실제 커맨드 결과를 나란히** 잰다.
  정상 출력이 나오는 것 중 가장 빠른 값을 `.env`에 넣으면 된다.
- `runuser`가 없는 서버(127)는 "커맨드가 없다"와 헷갈리지 않게 **별도 메시지**로 안내한다.
- 구 `SSH_SU_LOGIN=false`는 `su`로 계속 통한다(호환).
- `runuser`/`setpriv`를 차단 목록에 추가했다 — 강등 도구가 사용자 커맨드로 실행되면
  그 자체가 우회다(`su`/`sudo`는 이미 있었다).

**교훈.** "상주시키면 빠르다"는 맞지만, 무엇을 상주시키느냐가 다르다. 전송 연결은 상주시켜
이득만 가져오고, 셸은 상주시키지 않고 **셸을 여는 비용 자체를 없애는** 쪽이 옳다.

## 135. 로그인 셸을 안 거치는 것을 **안전하게** 켤 수 있게 (완료)

질문: *"그러면 우리도 로그인 셸을 안 거치고 할 수 있어?"* → **된다.** #134에서 이미 스위치를
만들었는데, "먼저 재 보고 바꾸세요"라는 단서가 붙어 있었다. 그 단서를 없앴다.

### 왜 단서가 붙어 있었나

`su`/`runuser`는 프로필을 안 읽으므로 PATH가 기본값이 된다. 사내 커맨드가 프로필에서
잡히는 경로에 있으면 **`command not found`로 바뀐다.** 그래서 바꾸는 것이 도박이었다.

### 실행 단계에서 스스로 되돌린다

비로그인 모드로 돌렸는데 커맨드를 못 찾으면(126/127 + stderr에 그 커맨드 이름),
**그 한 건만 로그인 셸로 다시** 돌린다. 그리고 커맨드 이름을 기억해 둔다 —
다음부터 그 커맨드는 처음부터 로그인 셸로 간다(두 번 돌지 않는다).

```
[ssh_exec] 'phd'은 로그인 셸이 필요합니다 → 이번 실행만 su -로 다시 합니다(다음부터는 처음부터).
[ssh_exec] phd 60ms (새 접속 · su-login · 202.20.185.100 · yr9.choi)
[ssh_exec] ls  17ms (새 접속 · su · 202.20.185.100 · yr9.choi)     ← 나머지는 계속 빠른 경로
```

대상 서버에 `runuser`가 아예 없는 경우(127)는 성격이 달라서 **모드를 통째로 `su`로 내린다**
— 커맨드마다 127을 반복할 이유가 없다. `.env`를 고칠 필요도 없다.

**두 가지가 겹칠 수 있다**(runuser도 없고 그 커맨드는 프로필도 필요한 경우). 처음엔 재시도를
한 번만 돌게 짰다가 테스트에서 **첫 실행이 그대로 실패하는 것**을 잡았다. 루프(최대 2회)로 고쳤다.

### 오탐을 막은 것

`command not found`만 보고 재시도하면, 사용자가 진짜 없는 커맨드를 부른 경우에도 매번 두 번씩
돈다. **stderr에 그 커맨드 이름이 함께 있을 때만** 재시도한다(`_looks_like_missing_command`).

### 결과

이제 `SSH_PRIVDROP=runuser`를 켜는 데 사전 확인이 필요 없다. 최악의 경우가
"프로필이 필요한 커맨드 한 종류가 처음 한 번 두 번 돈다"이고, 그 뒤로는 자동으로 최적 경로다.
결과 dict와 로그에 `privdrop`이 실려 나가므로 **어느 방식으로 돌았는지도 보인다.**
...
## 137. `rsync --delete`가 서버의 `.env`와 ssh 키를 지우고 있었다 (완료)

리포트: *"2. 에 있는 grep 하니까 .env 파일이 없다고 떠"*

`.env`가 왜 없나 보니 우리가 매번 지우고 있었다.

```
rsync -avz --delete --progress /home/yrc/AI-Infra-Assistant/ 서버:/…/AI-Infra-Assistant/
```

`--delete`는 **보내는 쪽에 없는 파일을 받는 쪽에서 지운다.** 그런데
- `.env`는 `.gitignore`에 있어 저장소에도 WSL에도 없다
- `secrets/`(ssh 개인키)도 저장소에 없다

→ **rsync 할 때마다 서버의 `.env`와 `secrets/`가 삭제됐다.** 우리가 준 절차 그대로 하면
지워지는 구조였고, CLAUDE.md·RUN-LOG·NEXT-STEPS 세 곳 모두 같은 커맨드였다.

### 왜 지금까지 안 터졌나 — 그리고 왜 다음 `up -d`에 터질 뻔했나

compose는 `${SSH_KEY_PATH:-./secrets/id_ed25519}`를 `/root/.ssh/id_ed25519`로 bind mount한다.
**바인드 마운트는 컨테이너를 만들 때 결정된다.** 그래서 호스트 파일이 사라져도 이미 떠 있는
컨테이너는 예전 키를 그대로 쥐고 있다 — `restart`로도 유지된다. 실제로 사용자의 bench는 잘 돌았다.

그런데 **`up -d`는 컨테이너를 재생성**한다. 그 순간 없는 경로를 마운트하게 되고,
도커는 그 자리에 **빈 디렉토리**를 만든다. `ssh -i <디렉토리>`는 무조건 인증 실패다.
#57에서 겪은 그 증상이고, NEXT-STEPS 2번이 정확히 그걸 실행시킬 참이었다.

### 조치

- **rsync에 `--exclude '.env' --exclude 'secrets/'`** 를 넣었다(CLAUDE.md·RUN-LOG·NEXT-STEPS 3곳).
  서버 전용 파일은 이제 rsync가 건드리지 않는다.
- `.gitignore`에 `secrets/` 추가 — 개인키가 저장소에 들어가는 것도 막는다.
- **복구 경로**: 키는 아직 **떠 있는 컨테이너 안에** 살아 있다. 재생성 전에 꺼내면 된다.
  `docker compose exec -T execution-mcp cat /root/.ssh/id_ed25519 > secrets/id_ed25519`
  NEXT-STEPS 2번을 이 순서로 다시 썼다(재생성 전에 키 복구 → 그다음 `up -d`).
- **조용히 실패하지 않게**: execution-mcp 기동 시 `SSH_KEY`가 정규 파일이 아니면
  "**모든 커맨드 실행이 인증 실패합니다**"를 로그 맨 앞에 크게 찍는다. 예전에는
  실행할 때마다 "권한 오류"로만 보여서 원인을 계속 다른 데서 찾았다(#57, #61, #62).

### 후속 — 키는 애초에 지워지지 않았다(경로를 잘못 짚었다)

사용자 확인: 호스트에 `/root/.ssh/id_ed25519`가 **그대로 있었다.** 즉 `.env`에
`SSH_KEY_PATH=/root/.ssh/id_ed25519`가 들어 있었고, 지워진 것은 `.env` 하나뿐이었다
(`secrets/`는 애초에 쓰지 않았다). 내가 "컨테이너에서 키를 꺼내라"는 복구 절차를 준 것은
불필요했고, 그 커맨드는 컨테이너가 Up이 아니면 조용히 빈 파일을 만든다("head가 안 나온다").

- compose 기본값을 `./secrets/id_ed25519` → **`/root/.ssh/id_ed25519`** 로 바꿨다.
  실제 배포 호스트에 존재하는 경로이므로, **`.env`가 없어도 키 마운트가 깨지지 않는다.**
  없는 경로를 기본값으로 두는 것 자체가 #57 이후 계속 함정이었다.
- `.env.example`도 `/root/.ssh/id_rsa`(존재하지 않는 파일)로 잘못 적혀 있어 함께 고쳤다.
- NEXT-STEPS에서 키 추출 단계를 빼고 `.env` 생성만 남겼다. `docker compose exec`가 조용히
  실패하는 경우(컨테이너가 Up이 아님)를 진단하는 커맨드도 대처표에 넣었다 — `ps`에는
  죽은 컨테이너가 안 나오므로 `ps -a`로 봐야 한다.

**교훈.** 배포 절차가 `--delete`를 쓰면, **저장소에 없는데 서버에는 있어야 하는 파일**이
무엇인지 먼저 세어 봐야 한다. 그리고 **기본값은 실제로 존재하는 경로로** 둔다 —
없는 경로를 bind mount하면 도커가 빈 디렉토리를 만들어 조용히 인증 실패로 바뀐다.
복구 절차를 줄 때도 "지금 무엇이 남아 있는지" 먼저 확인해야 한다. 이번엔 키가 멀쩡한데
꺼내라고 시켰다.

## 138. "되돌리기 실패: 405 Method Not Allowed" — 세 번째 같은 증상 (완료)

콘솔의 새 버튼(#136)을 눌렀더니 405. 코드는 멀쩡하다 — 라우트를 실제로 찍어 확인했다.

```
['GET']  /api/settings
['PUT']  /api/settings/{key}
['POST'] /api/settings/agent_system_instruction/reset   ← 등록돼 있다
```

**떠 있는 컨테이너가 예전 백엔드 코드라서** 그 라우트가 없는 것이다. FastAPI는 모르는 경로를
맨 끝 `app.mount("/", StaticFiles(...))`로 넘기고, StaticFiles는 GET/HEAD만 받으므로 405를 낸다.

### 왜 하필 이 증상이 반복되나 (#27, #30, 그리고 이번)

**화면과 백엔드의 반영 시점이 다르다.**
- `index.html`은 정적 파일이라 rsync하는 순간 새 코드다 → **새 버튼이 즉시 보인다.**
- 백엔드 파이썬은 uvicorn이 다시 읽어야 한다 → **재시작 전까지 그 API가 없다.**

그래서 "새 버튼이 보이는데 누르면 405"라는, 원인과 전혀 안 닮은 화면이 나온다.
세 번 다 같은 함정이었고, 세 번 다 "admin-console 재시작"이 답이었다.

### 조치 — 메시지가 다음 할 일을 말하게

`api()` 헬퍼에서 **`/api/` 경로의 405만** 따로 잡아 바꿨다.
...
앞으로 콘솔에 새 API를 붙일 때마다 같은 안내가 자동으로 나온다.
NEXT-STEPS 3번에도 `restart admin-console`을 넣었다 — `up -d`는 설정이 안 바뀐 컨테이너를
재생성하지 않으므로, 코드만 바뀐 이번 같은 경우엔 그냥 두면 그대로 남는다.

**교훈.** 같은 오해가 세 번 반복되면 문서에 한 줄 더 쓸 게 아니라 **에러 메시지를 고친다.**
사용자가 보는 화면이 원인을 말해야 한다.

## 139. 신뢰 경계에 인증이 없었다 + 홈 목록이 잘리는 이유 (완료)

요청: *"보안·인증 측면 강화하고, execution_mcp 기능 고도화하자. '내 홈스토리지에 어떤 파일들
있는지' 하면 리스트가 중간에 잘리는 것 같아. 제대로 로그인 서버에서 실행되는지 확인해줘."*

### (1) 보안 — 헤더를 그대로 믿는데 포트가 열려 있었다

구조를 다시 보면 이렇다.

```
Open WebUI ──(X-OpenWebUI-User-Email)──> agent-server ──(X-User-Id)──> MCP ── su - <그 계정>
```

두 화살표 모두 **헤더를 그대로 믿는다.** 그게 설계다(#3의 "user_id는 헤더에서 강제 주입").
문제는 그 포트들이 호스트에 열려 있었다는 것이다 — `8500`(agent), `8503~8505`, `8509`(MCP),
`8507`(postgres, dev 비밀번호는 고정값 `devpass`).

즉 **같은 망의 누구나** 이렇게 할 수 있었다.

```
curl :8504/mcp  -H 'X-User-Id: 남의계정'  ...   → 그 사람 권한으로 커맨드 실행
curl :8500/v1/agent/query -d '{"user_id":"남의계정", ...}'  → 같은 결과
curl :8507 (psql agent/devpass)                  → 설정·매뉴얼·VOC 전부
```

다중 사용자 시스템에서 이건 권한 상승이다. 세 겹으로 막았다.

- **`mcp_shared_secret`** — db-init이 `secrets.token_urlsafe(32)`로 **한 번** 심고
  (non-force라 이후 유지), agent-server와 MCP가 **같은 DB에서 읽는다.** agent-server가
  `X-Agent-Secret`으로 보내고 `CallerContextMiddleware`가 `hmac.compare_digest`로 확인한다.
  관리자가 손댈 값이 아니다 — 설정 화면에도 안 띄운다.
  **비밀값이 아직 없는 배포는 통과시킨다**(돌던 서비스를 갑자기 세우지 않는다).
  틀렸을 때의 401 본문에 "agent-server가 예전 코드면 이 오류가 난다 → restart-mounted.sh"를
  적어 뒀다 — 롤링 재시작 중의 혼란을 줄이려는 것이다.
- **`agent_api_key`** — Open WebUI 연결(Connections)에 넣은 키와 같은 값을 콘솔에 넣으면
  `/v1/models`·`/v1/chat/completions`·`/v1/agent/query`·`/v1/voc/query` 전부에 인증이 걸린다
  (하나라도 빠지면 우회로가 되므로 테스트로 고정했다). **비우면 인증이 없고**, 그 사실을
  기동 로그에 경고로 남긴다 — 조용히 열어 두지 않는다.
- **내부 전용 포트를 `127.0.0.1`에만 연다**(postgres + MCP 4개). 서버에서 `curl localhost:8504/warm`
  같은 진단은 그대로 되고, 망의 다른 장비에서는 닿지 않는다. agent(8500)·콘솔(8501)·
  Open WebUI(8502)는 그대로 둔다(8500은 외부 agent 연동 예정, 콘솔은 자체 로그인이 있다).

### (2) 홈 목록이 잘리는 이유 — 셋 중 하나인데 구분이 안 됐다

후보가 셋이었다: ① `ls`에 `-A`가 없어 숨김 파일이 빠졌다 ② 출력이 4,000자에서 잘렸다
③ 모델이 행을 줄였다. **화면에 아무 단서가 없어서** 구분할 수 없었다.

- 잘림을 **구조화된 값**으로 돌려준다(`truncated`/`total_lines`/`shown_lines`).
  예전에는 안내 문구를 stdout 끝에 붙이는 게 전부라, 모델이 그 줄을 빼먹으면 사용자는
  목록이 전부인 줄 알았다. 이제 진행 줄에 **LLM을 거치지 않고** 찍힌다:
  `· 완료 (202.20.185.100 · yr9.choi · 0.4초) ⚠ 출력 132줄 중 58줄만`
- 실행한 커맨드를 **통째로** 보여준다: `· \`ls -lh\` 실행하는 중`.
  `-A`가 빠진 건지(①) 잘린 건지(②) 한눈에 갈린다. 서버 로그에도 argv 전체를 찍는다.
- 잘렸을 때 stdout 끝 문구도 "전체 N줄 중 M줄만 보입니다 … **'일부만 표시됐다'고 반드시
  밝히세요**"로 강하게 바꿨다.

**"로그인 서버에서 도는지"는 이미 화면에 있다** — 진행 줄의 `(202.20.185.100 · yr9.choi)`가
그 증거다. 모델이 쓴 "…/home/gpu1/yr9.choi 기준입니다" 같은 문장은 근거가 아니다(#125에서
그런 문장을 지어낸 적이 있다). IP·계정·초·강등 방식은 전부 실행 결과에서 온 값이다.

---

## 140. 남의 계정 조회를 "모델이 거절해 주기"에 맡기고 있었다 + 엑셀 양식 (완료)

사용자 질문: "사용자가 다른 사람 id로 커맨드 실행하고자 할 때 막아야 하는데, 현재는 어떻게
막고 있는지?" — 답을 정리하다 **실제로는 막고 있지 않은 구간**이 나왔다.

### (1) 지금까지 막고 있던 것 — *실행 신원*

| 층 | 무엇을 막나 |
|---|---|
| Open WebUI → agent-server | 로그인한 계정을 `X-User-Id`로 붙인다. LLM이 만드는 값이 아니다 |
| agent-server → MCP | `X-Agent-Secret` 공유 비밀값(#139). 같은 망에서 헤더만 위조해 부를 수 없다 |
| MCP 툴 스키마 | `user_id`를 **스키마에서 제거**한다. LLM은 이 파라미터의 존재조차 모른다 |
| `build_wrapped` | 그래도 값이 들어오면 버리고 헤더 값으로 덮는다. 헤더가 없으면 거부(fail-closed) |
| `ssh_exec` | `ssh root@host` → `runuser -u <호출자>`. 원격에서 **본인 계정으로 강등**해 실행 |

그래서 파일 권한처럼 **OS가 판정하는 것**은 안전하다. 남의 홈은 읽히지 않는다.

### (2) 막고 있지 않던 것 — *커맨드 인자*

`phd list -u cocoa.song`은 본인 계정으로 실행해도 **남의 job을 보여준다**. 판정 주체가 OS가
아니라 그 프로그램이기 때문이다. 실제 답변에서 에이전트가 거절하기는 했지만 그건 지시문을
따른 **모델의 선택**이었을 뿐이다. 게다가 등록 커맨드 `phd list {option}`의 `{option}` 한 자리에
`-u cocoa.song`을 통째로 넣으면 자유 인자 검사도 비껴간다(토큰 하나로 들어온다).

→ `reject_other_user()`를 실행 직전에 넣었다. `-u 남`, `--user=남`, `-u남`(붙여 쓴 꼴), 등록
커맨드의 자리표시자 — **네 경로 모두** 차단하고 본인 계정(`-u yr9.choi`)만 통과시킨다.
설정 `execution_user_scope_flags`로 조정한다(`sort -u`처럼 계정과 무관한 `-u`가 걸리면 뺀다).

### (3) 답변이 사용자를 `ops_assistant`라고 불렀다

> S2 스케줄러는 현재 로그인한 사용자(**ops_assistant**)의 job 정보만 조회할 수 있으며…

`ops_assistant`는 ADK가 시스템 프롬프트에 넣는 **에이전트 자기 이름**이다. #125·#131과 같은
뿌리인데, 앞의 둘은 지시문으로 막으려 했고 또 재발했다. 이번에는 원인을 없앴다 — **호출자
계정을 알려 준 적이 없었다.** `build_agent`의 '이 환경의 값' 블록에 `질문한 사용자 계정`을
넣는다. 모를 때 지어내지 말라고 하는 것보다, 아는 값을 주는 쪽이 맞다.

### (4) 거절한 뒤 가이드 문서를 안내했다

"가이드 위치: 슈퍼컴 Portal > 활용 가이드 > …", "운영팀에 문의하세요"가 따라붙었다. 물어본
것은 남의 job이지 문서가 아니다. 지시문에 `## 남의 계정을 묻는 질문 — 한 줄로 끝냅니다`를
넣고, **거부 오류 문구 자체가** 다음 행동을 지시하게 했다("매뉴얼을 찾지 말고, 가이드 문서
위치를 안내하지 말고, 다른 방법을 제안하지 마세요").

### (5) 등록한 인자 설명이 LLM에 **한 글자도** 가지 않고 있었다

사용자가 "`{option}`인 경우 등록하기 조금 어려움"이라고 한 것의 진짜 원인. `build_entry`가
`inspect.Parameter(annotation=str)`만 만들어서, 콘솔에 적어 둔 "`-j`: JSON 형식으로 반환 …"이
스키마에 실리지 않았다. 등록은 되는데 에이전트가 옵션을 못 채우는 게 당연했다.

→ `Annotated[T, Field(description=…)]`로 바꿨다. 선택형은 `Literal`로 줘서 스키마에 enum이
박힌다. 선택지를 **`값: 설명`**(`-j: JSON 형식으로 반환`)으로 적을 수 있게 하고, 값만 뽑는
규칙(`choice_value`)을 `cast_arg`와 공유한다 — 스키마와 검증이 어긋나면 전부 거부된다.
필수가 아닌 선택형에는 빈 값도 enum에 넣는다(기본값 `""`이 없으면 pydantic이 스키마를
만들다 죽고 **툴 하나가 통째로 사라진다**).

### (6) 엑셀 양식

예전 일괄 등록은 이름·설명·실행 커맨드 세 열만 읽고 **인자를 버렸다**. 자리표시자가 있는
커맨드는 엑셀로 넣어 봐야 빈 인자로 들어가 결국 한 건씩 화면에서 다시 채워야 했다.

- `GET /api/execution/commands/template.xlsx` — `mode=template`은 예시 3행이 든 빈 양식,
  `mode=current`는 **현재 등록분을 같은 양식으로** 내보낸다. 고쳐서 그대로 올리면 이름 기준
  덮어쓰기 = '엑셀로 수정'.
- 인자는 `인자1 이름/설명/타입/필수/기본값/선택지` … 4벌까지 열로 편다.
  **이름 칸은 비워도 된다** — 실행 커맨드의 자리표시자 순서로 자동 연결한다(옮겨 적다 틀리면
  그 커맨드만 조용히 거부되던 부분).
- 선택지 구분자는 줄바꿈 또는 `|`. **콤마는 쓰지 않는다** — 설명에 콤마가 흔하다
  ("Return job info, json format").
- 이름·설명·실행 커맨드는 예전처럼 열을 골라 받고(머리글이 제각각인 사내 표), 인자·실행
  위치·활성·역할은 머리글로 알아본다. 고를 열이 서른 개가 되면 매핑 화면이 못 쓸 물건이 된다.
- 일괄 등록도 이제 화면 등록과 **같은 `validate_definition`** 을 거친다.

---

## 141. dev postgres에 데이터 볼륨이 없어 DB가 통째로 날아갔다 (원본 볼륨 확인, 복구 절차 확정)
...
### 원본을 찾았다 — 진단이 두 번 거짓말을 했다

첫 진단(`docker volume ls -f dangling=true` + `docker run alpine`)은 **아무것도 못 찾았다.**
데이터가 없어서가 아니라 진단이 틀려서였다.

- `docker run alpine` — **폐쇄망은 alpine을 pull하지 못한다.** 게다가 `2>/dev/null`로 에러를
  가려서 "볼륨이 없다"와 "이미지를 못 받았다"가 똑같이 **빈 출력**으로 보였다.
  → 로컬에 이미 있는 pgvector 이미지를 쓰고, 실패하면 원문을 그대로 찍는다.
- `dangling=true` — 멈춘 컨테이너에 붙어 있는 볼륨은 dangling이 아니라 목록에 안 나온다.
  → 볼륨 **전체**를 훑는다.
- `find-lost-db.sh`가 "postgres 컨테이너를 찾지 못했습니다"라고 찍었는데 실제로는 healthy로
  떠 있었다. `cd "$(dirname "$0")/.."`가 원인 — 스크립트를 저장소 루트에 복사해 두고 실행하면
  한 단계 위로 올라가 compose 파일을 못 찾는다. **진단이 거짓말을 하면 없느니만 못하다.**

고친 진단으로 PG16 볼륨 3개가 나왔고, `inspect-pg-volume.sh`로 내용을 열어 갈랐다.
**크기로 고르지 않았다** — 빈 클러스터도 130MB다.

| 볼륨 | 매뉴얼/청크 | VOC | 커맨드 | 마지막 활동 |
|---|---|---|---|---|
| `4f7c8f2e…` 2.4G | **5 / 542** | **48,314** | **3** | 08-06 02:59 |
| `553dd706…` 132M | 0 / 0 | 0 | 0 | 08-06 06:31 (재시드, 현재 붙어 있던 것) |
| `1dc7527f…` 137M | 1 / 175 | 0 | (표 없음) | 07-28 |

`4f7c8f2e…`가 원본이다. 등록 커맨드 3개(`myquota`·`s2_phd_info_job_id`·`s2_phd_list`)가
사용자가 Errors에 적어 둔 목록과 정확히 일치하고, job_logs가 사고 당일 02:59까지 찍혀 있다.
06:31에 빈 볼륨이 새로 만들어진 시각과도 맞아떨어진다.

복구는 **덤프 먼저**(`pg-rescue-<날짜>.sql`) → `ai-infra-assistant_pg_data_dev` 생성 →
`cp -a` → `up -d` 순서다. 원본 볼륨은 지우지 않고 남긴다.
...
## 143. `/v1/memory/*` 세 개가 인증 없이 열려 있었다 (완료)

#139에서 `/v1/*`에 인증을 걸 때 **네 개만 골라서** 걸었다. `/v1/memory/{user_id}`
GET/POST/DELETE 셋이 빠졌고, 코드 주석에는 `# 장기 메모리 관리 (인증 없음, 내부망 전용)`이라고
적혀 있었다 — `_caller_from_request`에 있던 것과 **똑같은 틀린 전제**다. 8500 포트는
0.0.0.0에 열려 있다(외부 VOC agent가 붙어야 해서 닫을 수 없다).

셋 중 **POST가 가장 위험하다.** 여기 넣은 내용은 `_memory_context`를 거쳐 그 사용자의 다음
대화부터 **시스템 지시문에 붙는다**(`extra_instruction`). 남의 에이전트에 영구적인 지시를
심을 수 있다는 뜻이다. GET은 남의 대화에서 증류된 내용을 읽고, DELETE는 통째로 지운다.

### 왜 놓쳤나 — 테스트가 목록을 박아 뒀다
...
#139에서 내가 쓴 회귀 테스트다. **내가 인증을 건 것만 검사한다** — 그러니 빠뜨린 것을
영원히 못 잡는다. 테스트가 있다는 사실이 오히려 안심시켰다.

→ 소스에서 `@app.<verb>(...)`를 **전부 열거**해 인증이 없는 것을 찾도록 바꿨다. 의도적으로
여는 경로는 `exempt`에 이유와 함께 적는다(지금은 `/health` 하나 — 신원도 실행도 없고
`restart-mounted.sh`가 기동 확인에 쓴다). 인증 제거·새 무인증 엔드포인트 추가 세 경우로
깨뜨려서 전부 잡히는 것을 확인했다.

### 교훈

**"무엇을 고쳤는지"를 검사하는 테스트는 "무엇을 빠뜨렸는지"를 못 잡는다.** 보안 검사는
화이트리스트(열거 + 예외 목록)로 써야 한다. #142와 같은 뿌리다 — 그때는 제약을 걸면서
기존 *호출자*를 안 훑었고, 이번엔 제약을 걸면서 기존 *대상*을 안 훑었다.

---

## 144. `ls`가 `/root`에서 돌아 Permission denied — 비로그인 강등의 작업 디렉토리 (완료)

사용자: "`내 홈 파일 리스트 보여줘` 하니까 `ls: cannot open directory '.': Permission denied`"

### 원인

`ssh root@host <cmd>`는 **root의 홈(`/root`)에서 시작**한다. 그리고 강등 방식마다 작업
디렉토리 처리가 다르다.

| 방식 | 작업 디렉토리 |
|---|---|
| `su - user -c` (로그인 셸) | **홈으로 이동** |
| `su user -c` (비로그인) | 그대로(`/root`) |
| `runuser -u user --` | 그대로(`/root`) |

#134에서 속도 때문에 `SSH_PRIVDROP=runuser`를 권했고(로그인 셸 ~2초 절약), NEXT-STEPS에도
그렇게 적었다. 그 순간부터 `ls -lh`는 **`/root`를 열려고 했고**, 일반 계정이라 당연히
`Permission denied`가 났다. `myquota`·`phd list`처럼 경로가 필요 없는 커맨드는 멀쩡히 돌아서
증상이 `ls` 계열에만 나타났다 — 그래서 늦게 발견됐다.

지시문에는 **"실행은 항상 본인 계정의 홈에서 시작합니다"** 라고 적혀 있었다. 그 약속이
`su-login`일 때만 참이었던 것이다.

### 조치

`_with_home_cwd()`: 비로그인 모드에 `cd ~user 2>/dev/null; ` 를 앞에 붙인다.

- `~user`는 **root 셸의 틸드 확장**이다. 계정명은 `_USER_RE`(`^[a-z_][a-z0-9_.-]{0,63}$`)로
  검증돼 셸 메타문자가 없으므로 따옴표 없이 안전하다(따옴표를 붙이면 확장이 안 된다).
- **`&&`가 아니라 `;`인 것이 핵심이다.** GPFS root_squash처럼 root가 사용자 홈에 못 들어가는
  환경에서 `&&`면 커맨드가 아예 실행되지 않는다. `;`면 그런 환경에서 **예전과 똑같이** 돌고,
  들어갈 수 있는 환경에서만 고쳐진다 — 회귀 없이 개선만 남는다.
  로컬 셸에서 실제로 확인했다: 없는 계정에 `&&`면 종료코드 2에 출력 없음, `;`면 정상 실행.
  sh·bash·dash 모두 `~user`를 확장한다.
- 인자 인용은 그대로다(`; rm -rf /`는 여전히 따옴표 안).

### `echo $HOME`은 애초에 동작하지 않는다

사용자: "홈 디렉토리 경로는 `echo $HOME` 으로 확인해서 결과 반환해야지."

맞는 지적인데, **`echo $HOME`은 우리 구조에서 `$HOME`이라는 글자를 그대로 출력한다.**
셸 없이 argv로 실행하기 때문에 변수 확장이 일어나지 않는다. 지시문에 그 사실을 못박고
(`$HOME`·`$USER`·`~`·`*`·`|`·`>` 금지), **경로를 물으면 `pwd`를 실행해서 답하라**고 적었다.
기존 지시문은 `pwd`를 "앞세우지 마라"고만 해서, 모델이 *직접 물었을 때도* 안 돌리고
"일반적으로 /home/... 입니다"라고 추측했다 — 확인용 커맨드를 앞세우지 말라는 것과
사용자가 물은 것을 확인하지 말라는 것은 다르다.

### 교훈

**지시문이 시스템 동작을 서술할 때, 그 서술이 참인지 코드로 확인한다.** "항상 본인 홈에서
시작합니다"는 내가 #128에 쓴 문장인데, #134에서 강등 방식을 바꾸면서 그 전제가 깨진 것을
보지 못했다. 설정 하나(`SSH_PRIVDROP`)가 지시문의 사실 관계를 바꿀 수 있다는 것을 놓쳤다.

---

## 145. 지시문에 커맨드를 박는 습관을 코드로 막았다 (완료)

사용자: "아니 지시문에 pwd를 명시하면 안되지... 하나하나 다 일일이 명시할거야?
**절대 지시문에는 특정 커맨드를 명시하거나 하지 마.**"

#144를 고치면서 지시문에 `pwd`·`whoami`·`find`를 적어 넣었다. **네 번째다**
(#74 `phd info -u`, #125, #140, 그리고 이번). 매번 "이번 건은 예외"라고 생각했는데 아니었다.

### 왜 하면 안 되나

커맨드는 두 곳에서 온다 — **콘솔 등록분**(툴로 노출되고 설명·인자가 스키마에 실린다)과
**모델이 아는 표준 리눅스 명령**. 지시문은 어느 쪽도 아니다.

지시문에 적으면 (1) 하나를 적는 순간 나머지도 다 적어야 하고, (2) 시스템이 바뀌면 지시문이
**거짓말을 한다.** 후자가 실제로 났다 — "실행은 항상 본인 홈에서 시작합니다"는 내가 #128에
쓴 문장인데, #134에서 `SSH_PRIVDROP`을 바꾸자 거짓이 됐고 그게 #144다.

### 조치

- 지시문에서 커맨드를 전부 걷어냈다: `pwd`·`whoami`·`find`·`echo $HOME`·`ls -l`·`ls -lh`·
  `ls·df·du·find·head·nvidia-smi` 나열·`(ls, grep, tar, awk 등)`.
- 대신 **성질**을 말한다: "커맨드는 셸 없이 argv 그대로 실행됩니다. 그래서 셸 변수(`$...`),
  틸드, 와일드카드, 파이프, 리다이렉션은 동작하지 않고 **글자 그대로** 전달됩니다."
  이러면 모델이 `echo $HOME`을 스스로 피한다 — 무엇을 쓰라고 시킬 필요가 없다.
- "그 값 자체를 물으면 실행해서 나온 값으로 답한다"는 원칙만 남겼다(커맨드는 안 정한다).
- 회귀 테스트 `test_instruction_names_no_specific_commands`: 금지 토큰 목록으로 지시문 본문을
  검사한다. `pwd`/`phd`/`$HOME`을 다시 넣어 보고 셋 다 잡히는 것을 확인했다.
- CLAUDE.md §3에 규칙으로 못박았다.

### `echo $HOME`은 왜 안 되나 (검증함)

사용자: "로그인 서버에서 사용자 계정으로 `echo $HOME` 하면 홈 스토리지가 나오잖아."

대화형 셸에서는 맞다. 그런데 우리 경로에서는 안 된다. 실제로 돌려서 확인했다.

```
su        원격 문자열: cd ~root 2>/dev/null; su root -c 'echo '"'"'$HOME'"'"''
사용자 셸이 실행한 결과: '$HOME'      ← 글자 그대로
```

`_remote_command`가 인자마다 `shlex.quote`를 걸기 때문이다. 홑따옴표 안이라 확장이 일어나지
않는다. **이건 의도된 것이라 풀 수 없다** — 풀면 `$(rm -rf /)`도 같이 열린다.
`su -c`는 셸을 거치지만 인자가 인용돼 있어 결과는 `runuser`와 같다.

홈 경로는 #144의 작업 디렉토리 수정으로 이미 얻을 수 있다(홈에서 시작하므로 현재 경로를
출력하면 된다). 어떤 커맨드를 쓸지는 모델이 고른다.

---

## 146. 답변이 행을 조용히 줄인다 — 출력 충실도 (진행 중)

사용자 리포트 세 건. **실행은 전부 정상이고, 셋 다 답변 품질 문제다.**

| 질문 | 증상 |
|---|---|
| 내 홈 파일 리스트 | `ls -la`는 잘 돌았는데 **16행까지만** 표출 |
| 내 홈 디렉토리는 어디야? | `/home/yr9.choi` — **지어냈다**(실제는 `/home/gpu1/yr9.choi`) |
| 내 s2 gpu job 리스트 | 표로 안 만들고 `phd list -a` 원문을 그대로 붙임 |

### (1)(3) 우리가 자른 게 아니다 — 모델이 잘랐다

`ls -la` 16행이면 약 1,200자다. `execution_result_max_chars`가 4000이므로 **우리 잘림 상한에
한참 못 미친다.** 잘렸다면 진행 줄에 `⚠ 출력 N줄 중 M줄만`이 떴을 텐데 그 보고도 없었다.
따라서 모델이 임의로 줄인 것이다.

기존 지시문은 이렇게만 적혀 있었다: "값을 바꾸거나 빠뜨리지 않습니다 - 행을 줄이려면
줄였다고 밝힙니다." **한 줄에 두 규칙을 욱여넣어서 약했다.** 강화:

- "행을 임의로 줄이지 않습니다. **이게 가장 자주 나는 사고입니다.**"
- 도구 결과에 `전체 N줄 중 M줄만 보입니다`가 붙어 있을 때**만** 잘린 것이다. 그 안내가
  없으면 받은 것이 전부이므로 전부 보여준다 — 즉 **판단 기준을 우리가 붙인 마커로 고정**했다.
- 줄여야 할 만큼 길면 **먼저 밝히고** 줄인다. 조용히 줄이는 것이 문제다.
- "원본이 이미 읽기 좋은 형태면 그대로 둔다" — `이름: 값`으로 늘어선 상세 출력을 억지로
  표로 접으면 오히려 나빠진다(#146 이전엔 무조건 표로 만들라고만 했다).

### (3) `-a`는 모델이 지어낸 옵션이다

`phd list {option}`의 `option`이 아직 타입 '문자열'이라 무엇이든 들어간다. #140에서 만든
**선택형(enum)** 으로 바꾸면 스키마에 값이 박혀 지어낼 수 없다. 엑셀로 재등록하는 것이
NEXT-STEPS 4번이다. 지시문으로 막을 일이 아니라 **스키마로 막을 일**이다.

### (2) 홈 경로 — 먼저 지시문 버전부터 확인한다

옛 지시문은 "`pwd` 같은 확인용 커맨드를 앞세우지 마세요"라고 **적극적으로 막고** 있었다.
그 상태면 모델이 실행하지 않고 추측하는 것이 오히려 지시를 따른 결과다. #144/#145에서
고쳤지만 지시문은 non-force 시드라 **콘솔에서 되돌리기 버튼을 눌러야** 반영된다.

그래서 원인을 추측하기 전에 **배포된 지시문 버전을 DB에서 직접 확인**하는 커맨드를
NEXT-STEPS 1번에 넣었다.

```sql
select case when value like '%행을 임의로 줄이지 않습니다%'
         then '최신 지시문 OK' else '옛 지시문 - 되돌리기 필요' end
  from platform_settings where key='agent_system_instruction';
```

**이걸 먼저 만들었어야 했다.** #144·#145를 고칠 때마다 "되돌리기 버튼을 누르세요"라고 적었을
뿐, 눌렀는지 확인할 방법을 주지 않았다. 그래서 "고쳤는데 왜 그대로냐"를 두 번 겪었다.

### 곁가지로 잡은 것 — 테스트가 잘못된 문서를 물고 있었다

`test_console_explains_405_instead_of_showing_it`이 **NEXT-STEPS에** "restart admin-console"이
있는지 검사하고 있었다. NEXT-STEPS는 매 턴 "지금 할 일"만 남기고 새로 쓰는 문서라
(CLAUDE.md 1절), 이번처럼 콘솔을 안 건드리는 턴에는 그 줄이 사라져서 테스트가 깨진다.
**영구 규칙을 임시 문서에 걸어 둔 것**이 잘못이다. 사용자가 매번 돌리는
`scripts/restart-mounted.sh`가 admin-console을 재시작하는지 검사하도록 옮겼다.

(이걸 발견한 경위도 남긴다: `pytest | tail -2 && git commit`으로 묶어 돌리는 바람에
**파이프가 종료코드를 가려** 실패한 테스트를 그대로 푸시했다. 다음 커밋에서 고쳤다.)

### 아직 안 한 것 (의도적으로)

홈 경로를 '이 환경의 값'에 넣는 방안(#140에서 계정명으로 효과를 본 방식)을 검토했다.
구현하려면 홈 경로를 원격에서 한 번 조회해 캐시해야 하는데, 그건 **첫 커맨드에 지연을
얹는다**(#128~#134에서 줄인 그 지연이다). 지시문 반영 여부가 확인되기 전에 기계장치를
먼저 만들면 원인을 못 짚은 채 복잡도만 늘어난다 — 1번 확인 결과를 보고 정한다.

---

## 147. "되돌리기" 버튼이 **옛 지시문을 계속 저장**하고 있었다 (완료)

사용자: "2번 그대로 했는데도 1번 하면 옛지시문이라고 떠"

### 원인 — 파이썬 모듈 캐시

`reset_agent_instruction`이 함수 안에서 이렇게 읽고 있었다.

```python
from agent_instruction import AGENT_INSTRUCTION   # noqa (부수효과 없는 모듈)
```

주석에 "부수효과 없는 모듈"이라고 써 뒀는데, **부수효과가 없는 것과 최신인 것은 다르다.**
파이썬은 `sys.modules`에 캐시하므로, 이 프로세스가 예전에 한 번이라도 이 모듈을 읽었으면
그 뒤로는 파일이 아무리 바뀌어도 **캐시된 옛 텍스트**를 돌려준다.

`./shared`가 바인드 마운트라 rsync만 하면 파일은 최신이 된다. 그래서
"코드는 최신 → 버튼을 눌렀다 → 그런데 DB에는 옛 지시문"이라는, 원인을 짐작하기 어려운
상태가 됐다. #144·#145를 고칠 때마다 "되돌리기를 누르세요"라고 안내했는데 **그 버튼 자체가
고장나 있었으므로 두 수정이 모두 서버에 반영되지 않았다.**

### `importlib.reload`도 부족하다

reload로 `sys.modules`는 피할 수 있지만, `.pyc` 바이트코드 캐시는 **mtime + 파일 크기**로
유효성을 판단한다. 같은 초에 같은 크기로 바뀌면 낡은 바이트코드를 그대로 쓴다.
실제로 재현했다(8자 → 8자로 바꾸니 reload가 옛 값을 돌려줬다).

### 조치

`_read_instruction_from_disk()` — 소스를 **직접 읽어 `compile`** 한다. 우리가 읽은 문자열을
컴파일하는 경로는 pyc 캐시를 타지 않으므로 항상 최신이다. `agent_instruction.py`는
docstring과 문자열 상수뿐이라 exec해도 안전하다.
읽지 못하면 **조용히 옛 값을 쓰지 않고 500으로 실패**한다 — 조용한 실패가 이 버그의 본질이었다.

바이트 수가 같은 한글 치환으로 최악 조건을 만들어 확인했다.

```
① import 캐시 : False  ← 못 본다 (운영에서 난 버그)
③ 파일 직접   : True   ← 항상 본다
```

### 곁가지 — 내 테스트가 버그를 요구하고 있었다

#136에서 쓴 `test_instruction_can_be_reset_from_console_without_db_env`가
`assert "from agent_instruction import AGENT_INSTRUCTION" in router` 를 검사하고 있었다.
**즉 방금 제거한 버그 패턴을 테스트가 강제하고 있었다.** 그 테스트의 원래 의도(콘솔이
`POSTGRES_PASSWORD` 없이 지시문을 읽을 수 있어야 한다)는 유지하고, 검사 대상을 새 방식으로
바꿨다. 구현 방법을 그대로 베낀 테스트는 구현이 틀렸을 때 같이 틀린다.

### 교훈

**"반영했는데 안 된다"가 반복되면 반영 경로 자체를 의심한다.** #146에서 지시문 버전 확인
커맨드를 만든 것은 옳았지만, 거기서 한 걸음 더 가서 "확인이 계속 실패하면 버튼을 의심한다"까지
갔어야 했다. 확인 수단을 만든 덕분에 이번엔 두 턴 만에 잡혔다.

---

## 148. "어디야?"는 실행 대상이 아니라고 분류하고 있었다 + 엑셀 드롭다운 (진행 중)

사용자: "첫번째 홈 파일 리스트 보여달라고 할 때는 잘만 로그인 서버에서 실행하더니
이 커맨드는 왜 자꾸 안되는거야?"

### 좋은 질문이고, 답은 "실행이 실패한 게 아니다"

두 질문은 **다른 갈래로 분류된다.**

| 질문 | 분류 | 결과 |
|---|---|---|
| "내 홈 파일 리스트 **보여줘**" | 실행 | 로그인 서버에서 잘 돈다 |
| "내 홈 디렉토리는 **어디야?**" | (B) 일반 지식 | **도구를 아예 호출하지 않는다** |

지시문 (B)절에 이렇게 적혀 있었다.

> 사용자가 자기 자원의 상태·내용을 **보여 달라/확인해 달라**고 하면 … 실제로 실행해서 …

"어디야?"는 "보여 달라"가 아니다. 그래서 모델은 (B) 일반 지식으로 보고 학습 지식에서
`/home/<계정>`을 만들어 냈다. **실행이 실패한 게 아니라 시도조차 안 한 것**이다 —
그래서 진행 줄에 실행 흔적이 없었을 것이다(다음 확인에서 이걸 본다).

→ (B)절에 물음 형태를 추가했다: "어디야? 뭐야? 얼마야? 같은 물음도 실행 대상이다.
'보여 달라'고 해야만 실행하는 것이 아니다." 그리고 **"일반적으로 …입니다",
"시스템에 따라 다를 수 있으나", "직접 확인해 보세요"를 쓰고 있다면 지금 실행해야 할 것을
안 한 것**이라고 못박았다. 커맨드 이름은 적지 않았다(#145).

`#144`의 홈 이동 수정과 합쳐지면, 실행만 하면 정확한 경로가 나온다.

### 잘림은 여전히 우리 쪽이 아니다

22행 표시에 `⚠` 없음 → `truncated=False` → 우리가 자르지 않았다.
`_clip`의 `total_lines` 설정과 `_result_phrase`의 `⚠` 렌더링 경로를 다시 읽어 확인했다
(둘 다 정상). 남은 변수는 **지시문이 실제로 반영됐는지** 하나뿐이라, NEXT-STEPS에서
확인을 통과하기 전에는 3번(품질 확인)으로 가지 못하게 순서를 고정했다.

### 엑셀 드롭다운 (사용자 요청)

타입·필수·활성·실행 위치를 자유 입력으로 두면 오타가 **조용히 다른 뜻**이 된다
("선택"이라고 적으면 선택형이 아니라 문자열로 들어간다). openpyxl `DataValidation`으로
드롭다운을 걸고 목록 밖 값은 거부한다(`errorStyle=stop`). 역할만 환경마다 달라
제안 목록 + 직접 입력을 허용했다(`errorStyle=warning`).

`인자N` 슬롯 4벌 전부에 걸리는지, 그리고 **드롭다운 값이 파서가 실제로 알아듣는 값인지**를
테스트로 고정했다 — 화면에서 고를 수 있는데 업로드하면 무시되는 값이 있으면 안 된다.

### 기본값을 비우면? (사용자 질문)

**그 인자가 커맨드에서 통째로 빠진다.** `render_argv`가 빈 문자열로 치환하고
`build_registered_argv`가 빈 토큰을 걸러낸다. 실제로 돌려 확인했다.

```
기본값 없음 + 값 없음 -> ['phd', 'list']
기본값 -l  + 값 없음 -> ['phd', 'list', '-l']
기본값 -l  + 값 -lf  -> ['phd', 'list', '-lf']
```

---

## 149. 실행 여부를 **말투로** 판단하고 있었다 + 지어내기 금지 안전장치 (완료)

사용자: "사용자가 어떻게 질문를 할지는 몰라. 항상 확인해달라, 보여달라 라고 말하지도 않을거야.
**문맥 상 시스템 정보가 필요한 경우면 execution_mcp가 실행되어야지.**
그리고 더 나아가서 … 일반지식이더라도 모델이 만들어내진 말아야지. 모르는 건 모른다고 해야지."

### (1) 질문 형태를 나열한 것이 잘못이었다

#148에서 나는 `"어디야?" "뭐야?" "얼마야?" 같은 물음도 실행 대상`이라고 **형태를 나열**했다.
커맨드를 나열한 #145와 **정확히 같은 실수**다 — 사용자가 어떻게 물을지는 알 수 없고,
목록에 없는 표현이 오면 또 뚫린다.

기준을 **내용**으로 바꿨다.

> **말투로 판단하지 않습니다.** 기준은 **답에 무엇이 필요한가** 하나입니다.
> · 답이 이 서버에 물어봐야 나오는 값이면 실행합니다. 판별법: 그 답이 **회사·서버·계정에 따라
>   달라지는가?** 달라진다면 당신은 그 값을 알 수 없고, 실행만이 답을 줍니다.
> · 어느 환경에서나 같은 답(명령어 문법, 개념, 오류 메시지의 뜻)이면 (B)입니다.

이 판별법은 **모델이 스스로 적용할 수 있는 형태**다. 목록이 아니라 질문 하나로 끝난다.

### (2) 마지막 안전장치 — 분류가 틀려도 지어내지는 않는다

분류는 확률적이라 언젠가 또 틀린다. 그때를 대비한 층을 (B)에 넣었다.

> **(B)로 답하더라도 이 환경의 값을 지어내지 않습니다.**
...
헤지 문구를 **금지 목록으로** 준 것이 핵심이다. 모델은 "지어내지 마라"는 추상적 지시보다
"이 문장을 쓰고 있으면 지어내는 중이다"라는 **자기 점검 신호**를 더 잘 따른다.
실제 사고 답변이 정확히 그 문구를 쓰고 있었다("일반적으로 /home/gpu1/yr9.choi 입니다"가
아니라 `/home/yr9.choi`를 지어내고 "일반적으로"를 붙였다).

### (3) 진행 줄에 줄 수를 **항상** 표시한다

잘렸을 때만 `⚠ N줄 중 M줄만`을 찍고 있었다. 그래서 22줄짜리 답을 받고도 **우리가 자른 건지
모델이 자른 건지 화면만 봐서는 알 수 없었다** — 사용자도 나도 그걸 두 턴 동안 못 갈랐다.

이제 잘리지 않아도 `· 132줄`이 붙는다. 답변의 행 수와 눈으로 대조하면 끝난다.
한 줄짜리 출력에는 붙이지 않는다(잡음).

### 남은 것

`ls -la` 22행 문제는 이 세 가지를 반영한 뒤 다시 봐야 한다. 지금까지의 리포트는
**#147(되돌리기 버튼 고장)이 고쳐지기 전** 것이라 지시문 변경이 서버에 반영된 적이 없다.

---

## 150. 실행 결과 원문을 **LLM을 거치지 않고** 답변 뒤에 붙인다 (완료)

사용자: "왜 애초에 잘려서 보이는거야?" → 확인해 보니 잘림 지점이 셋인데 **둘은 무관**했다.

| 지점 | 이번 건 |
|---|---|
| `execution_result_max_chars` 4,000자 | 22행이면 ~1,200자. **안 걸렸다**(걸렸으면 `⚠`가 떴다) |
| 모델 출력 토큰 상한 | **우리는 안 건다.** 132행 ≈ 2,600토큰, 여유 1만 토큰 이상 |
| **모델이 스스로 줄임** | ← 이것이 원인 |

`execution_result_max_chars`가 왜 있냐면 #123에서 프롬프트가 59,360토큰이 돼 32,768 컨텍스트를
넘겨 요청이 통째로 죽었기 때문이다(커맨드 출력은 다음 턴 이력에도 실린다).

### 모델이 왜 줄이나 — 절반은 내 탓이었다

첫째, LLM은 목록형 도구 출력을 "보기 좋게" 추리는 성향이 있다.
둘째, **지시문이 비대해졌다.**

```
지시문 12,751자 ≈ 7,787토큰 = 컨텍스트 32,768의 24%
```

이번 세션에서만 규칙을 계속 덧붙였다. "행을 줄이지 마라"는 그 7,787토큰 안의 **한 항목**이고,
지시문이 길수록 개별 규칙의 준수율은 떨어진다. **규칙을 추가해 고치려 한 것이 문제를 조금씩
키우고 있었다.**

### 조치 — 진행 줄과 같은 방식

지시문 수정에 세 번 의존했고 세 번 다 확실하지 않았다. 이 저장소에서 실제로 통한 것은
**LLM을 거치지 않는 경로**였다(진행 줄 — 모델이 뭘 쓰든 화면에 남는다).

`_RawOutputs`: 실행 결과 원문을 모아 **모델 답변 뒤에 그대로** 붙인다.

- 여러 줄일 때만(`execution_raw_output_min_lines`, 기본 2). 한두 줄은 답변에 이미 있다.
- 출력에 ``` 가 들어 있으면 **울타리를 늘린다**. 안 그러면 코드블록이 중간에 닫혀 나머지가
  마크다운으로 샌다.
- 원문 블록에도 상한을 둔다(`execution_raw_output_max_chars`, 기본 20,000).
  에이전트에 넘기는 상한(4,000)과 **별개다** — 화면에는 더 많이 보여줄 수 있다.
- 네 엔드포인트 × 스트리밍/비스트리밍 = **여섯 지점 전부**에 연결했다. 하나만 빠져도 그
  경로에서는 여전히 줄어든 답을 받는다(테스트가 개수를 고정한다).
- 검색 결과(`exit_code`가 없는 응답)는 건드리지 않는다.

`execution_raw_output_summary`(기본 **꺼짐**): 원문 뒤에 2~3줄 요약을 덧붙인다. 사용자가
"맨 마지막에 한 번 더 정리해 줘도 좋다"고 해서 넣었지만, **LLM을 한 번 더 부르므로 답변이 몇 초
늦어진다.** #128~#134에서 줄인 그 지연이라 기본은 꺼 뒀다. 실패하면 조용히 넘어간다.

### 남은 것

지시문 다이어트(7,787토큰 → 절반)는 아직 안 했다. 원문 블록이 결정적으로 동작하면 지시문의
출력 형식 규칙 상당수는 없애도 된다 — 반영 결과를 보고 정한다.

---

## 151. 확인 수단이 거짓말을 했다 — 매직 문자열로 지시문 버전을 보던 것 (완료)

사용자: "2번까지 그대로 했는데 아직 옛것 **12751자** 라고 나와"

**12,751자는 지금 코드의 지시문 길이 그대로다.** 즉 되돌리기는 진작 성공했고,
**내 확인 쿼리가 틀렸다.**
...
#149에서 사용자 지적("질문 형태를 나열하지 마라")대로 `어디야?` 나열을 **지웠는데**,
NEXT-STEPS의 확인 문자열은 #148 것을 그대로 뒀다. 그래서 **최신 지시문일수록 "옛것"이라고
나오는** 확인이 됐다. 사용자는 그 거짓 음성 때문에 한 턴을 버렸다.
...
|---|---|---|
| #146 | `셸을 거치지 않습니다` | (유지됨) |
| #148 | `행을 임의로 줄이지 않습니다` | (유지됨) |
| #149 | `어디야?` | **#149에서 내가 지움** |

문제는 운이 아니라 **방식**이다. "지시문에 이 문구가 있으면 최신"은 지시문을 고칠 때마다
같이 고쳐야 하는데, 고치는 사람(나)이 그걸 잊는다. #141(끝난 절차가 문서에 남아 재실행)과
같은 부류 — **문서에 사실을 복사해 두면 원본이 바뀔 때 거짓이 된다.**

### 조치 — 복사하지 말고 원본을 비교한다

`scripts/check-instruction.sh`: `shared/agent_instruction.py`를 **콘솔의 되돌리기 버튼과 같은
방식으로**(소스를 읽어 compile — 모듈 캐시를 타지 않는다, #147) 읽어 DB 값과 길이·앞뒤 200자를
비교한다. **지시문을 아무리 고쳐도 이 스크립트는 고칠 필요가 없다.**
...
**확인 수단을 만들 때, 그것이 검사 대상과 함께 낡는지 본다.** #146에서 확인 커맨드를 만든 것은
옳았지만 만드는 방식이 틀렸다. 사실을 복사한 확인은 원본이 바뀌면 조용히 거짓말한다 —
그럴 바엔 확인이 없느니만 못하다(없으면 최소한 의심이라도 한다).

---

## 152. 거짓 음성을 고치면서 새 거짓 음성을 만들었다 — 확인 스크립트 (완료)

사용자: "지시문 기본값으로 되돌렸는데, 스크립트 돌리니까 **길이는 같은데 내용이 다르다**고 나와."

#151에서 만든 확인 스크립트가 이렇게 비교하고 있었다.

```sql
select left(value,200) || '\x01' || right(value,200) ...   -- psql
```
```python
(t[:200] + "\x01" + t[-200:])                              -- 파이썬
```

`standard_conforming_strings=on`(9.1부터 기본)에서 psql의 `'\x01'`은 **리터럴 4글자**이고
파이썬의 `"\x01"`은 **1바이트 제어문자**다. 그래서 **길이가 같으면 무조건 "내용이 다르다"**
가 나왔다. #151에서 거짓 음성을 고쳐 놓고 **바로 다음 줄에서 새 거짓 음성을 만든 것**이다.

### 조치 — 이스케이프가 필요 없는 비교로

postgres의 `md5()`는 기본 내장이고(확장 불필요) UTF8 DB에서 파이썬의
`md5(t.encode("utf-8"))`와 **같은 값**을 준다. 구분자도, 조각 이어붙이기도 필요 없다.

**진짜 postgres 16을 띄워 확인했다**(추정으로 끝내지 않았다).

```
DB   : 12751 eb6144e24860739e1021b1ec57f7b264
코드 : 12751 eb6144e24860739e1021b1ec57f7b264      ← 일치
옛 방식으로 같은 데이터를 비교 → "다르다"          ← 버그 확정
```

세 갈래(일치 / 길이 같고 내용 다름 / 길이 다름)를 전부 돌려 메시지까지 확인했다.

### 교훈

**셸·SQL·파이썬 세 층을 문자열로 건너다니는 비교는 만들지 않는다.** 층마다 이스케이프 규칙이
달라서 조용히 어긋난다. 해시처럼 **16진수 문자만 오가는 값**으로 비교하면 층이 몇 개든 안전하다.

그리고 — 확인 도구를 만들 때도 **실제로 돌려 봐야 한다.** #151에서 나는 스크립트를 쓰고
문법만 확인했다(`bash -n`). 그래서 "항상 실패하는 비교"를 그대로 사용자에게 보냈다.

---

## 153. 지시문 재작성 — 라우팅 중심으로 절반 (완료)

사용자가 VOC 엑셀의 실제 문의("접속이 원활하지 않다, timeout으로 터미널을 못 본다")를 그대로
물었을 때 나온 세 가지 + "지시문 문제인지 MCP 툴 설명 문제인지 확인하자".

### 먼저 — **툴 설명 문제가 아니다**

`search_manual` / `search_voc` 설명을 읽어 보니 용도·인자·반환이 명확하다
("사용법·설정·절차·정책의 근거 문단", "증상·오류·장애용"). 문제는 지시문이었다.

### (1) 쓸데없는 실행 — 증상 호소를 상태 조회로 봤다

"접속이 안 된다"에 GPU job 목록을 실행해 보여줬다. 지시문은 **"값을 물으면 실행하라"** 는
말만 있고, **증상 호소는 값 요청이 아니라는** 구분이 없었다.

→ 판별을 표로 만들었다. **값**을 묻는다 → 실행 / **왜·어떻게**를 묻는다 → 매뉴얼·VOC 먼저 /
어느 환경에서나 같은 답 → 도구 없이. 그리고 못박았다: "증상을 호소하는 것은 상태 조회 요청이
아니다. 매뉴얼·사례를 먼저 보고, 거기서 무엇을 확인하라고 하면 그때 실행한다."
멈춤 조건도 명시했다 — **"답과 상관없는 도구를 부르지 않습니다."**

### (2) 멀티턴 오염 — 앞 턴의 거절을 새 질문에 적용했다

앞 턴에서 "다른 계정은 조회할 수 없다"고 거절한 뒤, 전혀 다른 VOC 문의에도 같은 거절을
반복했다. 기존 지시문에도 "질문이 바뀌면 다시 검색한다"가 있었지만 **검색 얘기였고 3절에
묻혀 있었다.**

→ 처리 절차의 **0단계**로 올렸다. "새 메시지가 오면 그 메시지 하나만으로 판단한다.
앞 턴의 주제·결론·**거절**을 이어붙이지 않는다. 대화 맥락은 대명사를 푸는 데만 쓴다."

### (3) 문서 안내를 코드 블록으로 출력

→ "**코드 블록은 실행한 커맨드와 그 출력에만** 쓴다. 문서 안내·절차 설명을 코드 블록에 넣지
않는다(사용자가 실행할 명령으로 오해한다)." 안내 형식에도 "코드 블록 없이"를 명시했다.

### 구조를 바꿨다 — 분류 중심 → **라우팅 중심**

기존은 (A)/(B)/(C) **지식 출처 분류**로 시작했다. 그런데 에이전트가 매 턴 실제로 내리는 결정은
**"어떤 도구를 부를 것인가"** 다. 그래서 2절을 처리 절차로 다시 썼다.

```
0) 이번 메시지만 보고 판단      ← 멀티턴 오염 차단
1) 무엇을 묻는지 가른다          ← 값 / 왜·어떻게 / 일반지식
2) 도구를 고른다                 ← 매뉴얼·VOC·실행, 순서 자유, 이어쓰기 가능
3) 멈춘다                        ← 상관없는 도구 금지
```

그리고 흩어져 있던 것을 한 곳에 모았다: "지어내지 마라"가 **13회 9개 섹션** → 1절 「절대 규칙」
6개로. "검색 결과 검증" 11회 → 한 절로. 사고 서술(내러티브)은 뺐다(HISTORY에 있으면 된다).

```
12,751자 / 7,787토큰 / 컨텍스트의 24%
 →  5,687자 / 3,542토큰 / 11%      (55% 감소)
```

**뺀 것은 중복과 서술뿐이고, 규칙은 하나도 안 뺐다.** 근거 규칙·조건 유지·값 변경 금지·
헤지 문구 금지·guide_location 원문 유지는 오히려 맨 앞으로 올렸다.

### 곁가지 — 내 테스트가 **제목**을 검사하고 있었다

세 테스트가 `## 남의 계정을 묻는 질문` 같은 **섹션 제목**과 `job 목록` 같은 **예시 단어**를
검사하고 있었다. 규칙이 살아 있는데도 실패했다. #147과 같은 실수 — 구현의 모양을 베낀 테스트는
구현을 고칠 때 같이 깨진다. **규칙을 검사하도록** 바꿨다.

새 테스트 5개로 이번 세 증상과 예산(<5,000토큰)을 고정했고, 네 가지로 깨뜨려 전부 잡히는 것을
확인했다.

---

## 154. 지어내기를 **코드로** 잡는다 — 근거 검사 (완료)

사용자: "없는 말은 지어내지 말아야 하는데 자꾸자꾸 지어냄. **지어내는건 치명적이니 강하게
막아둬야 함.**" 실제 사례가 심각했다.

- "login server 접속이 갑자기 안됩니다" → **없는 서버 IP를 만들어** 안내
- "접속이 안되면 슈퍼컴 접속 못 하는거 아니야?" → 매뉴얼에 접속 가이드가 있는데
  **manual_mcp를 전혀 부르지 않고** 클라이언트 사용법을 틀리게 안내
- 매뉴얼을 검색하지도 않고 "가이드 문서: 슈퍼컴퓨팅센터 사용자 매뉴얼"을 덧붙임

### 지시문으로 네 번 막았고 네 번 재발했다

#125·#131·#149·#153에서 "지어내지 마라"를 계속 강화했다. 그래도 난다. **지시문은 확률이다.**
#150에서 배운 것을 그대로 적용한다 — 사용자가 반드시 알아야 하는 것은 **LLM을 거치지 않고**
화면에 남긴다.

### `_Grounding` — 답변에 근거 없는 값이 있으면 지목한다

이번 턴의 **도구 결과 + 사용자 질문 + 우리가 프롬프트에 넣어 준 환경 값**을 근거 뭉치로 두고,
답변에서 다음을 뽑아 대조한다.

| 검사 | 왜 이것인가 |
|---|---|
| **IP 주소** | 사용자가 그 주소로 접속을 시도한다. 틀리면 곧바로 사고다 |
| **절대 경로** | 없는 경로를 안내하면 헤맨다(#125의 `/home/ops_assistant`) |
| **문서 안내 블록** | 매뉴얼을 검색하지 않았다면 그 블록 자체가 지어낸 것이다 |

근거에 없으면 답변 뒤에 경고를 붙인다. **답변을 고쳐 쓰지는 않는다** — 모델 문장을 기계가
손대면 더 이상해진다. 대신 **그 값을 지목**해서 사용자가 믿고 쓰지 않게 한다.

```
> ⚠️ 아래 내용은 근거가 확인되지 않았습니다 — 그대로 믿지 마세요.
> 조회 결과에 없는 값: `10.20.30.40`
```

우리가 준 환경 값(로그인 서버 IP 등)은 근거로 인정한다. 그러지 않으면 정상 답변마다 경고가
떠서 아무도 안 읽게 된다. 다섯 경우(지어낸 IP / 환경 값 IP / 검색 없는 문서 안내 / 지어낸 경로 /
정상 답변)를 돌려 확인했고, 여섯 지점(엔드포인트 3 × 스트리밍 2)에 전부 연결했다.

### 지시문에서 내가 약하게 만든 것을 되돌렸다

#153에서 지시문을 절반으로 줄이며 (A)절의 **도메인 범위**("접속·계정·스토리지·스케줄러·정책…")를
중복으로 보고 지웠다. 그런데 그건 중복이 아니라 **라우팅에 필요한 범위 정의**였다. 그래서
"접속이 안 된다"가 일반 지식으로 흘러 매뉴얼을 안 부른 것이다.

→ 라우팅 표 바로 밑에 되살렸다: **"이 회사 시스템에서 어떻게 하는가에 해당하면 매뉴얼 검색은
선택이 아니다. 검색하지 않은 채로 설명하지 않는다. 안 나오면 '매뉴얼에서 확인되지 않습니다'가
정답이다."** 그리고 "답과 무관한 문서 안내를 붙이지 않는다"도 추가했다(거절할 때 가이드를
덧붙이던 것).

**교훈**: 다이어트할 때 "중복"과 "범위 정의"를 구분해야 한다. 같은 말이 여러 번 나온다고 다
지우면, 그중 하나는 실제로 일을 하고 있었을 수 있다.

### 가이드 위치가 제목만 나오는 것 (사용자 지적 3)

`guide_location`은 `manual_files.reference_path`에서 온다. **관리자가 그 값을 안 채웠으면 빈
문자열**이라 지시문대로 "비어 있으면 문서명만" 쓴 것이다. 코드 문제가 아니라 **데이터 문제** —
콘솔 매뉴얼 탭에서 문서마다 위치를 채워야 한다. NEXT-STEPS에 넣었다.

---

## #155. 매뉴얼을 **우리가 먼저 검색**하고, 근거 없는 답변은 **버린다**

사용자 지시(그대로):

> 무조건 manual_mcp 사용해서 그 기반으로 대답하게끔 해줘. **저런 그대로 믿지마세요 문구를
> 넣지말라고. 아예 지어내지 말라고.** 우리 매뉴얼에 없거나 그 어떤 db에서 확인할 수 없는거면
> **운영팀에 문의하라**고 하라고. 콘솔에 매뉴얼 reference path 다 있는데 agent가 사용을 안한다고.

세 가지를 지적했고 셋 다 맞다. 하나씩.

### (1) 내가 #154에서 내린 진단이 틀렸다 — "데이터 문제"가 아니었다

#154에서 "가이드 위치가 안 나오는 건 관리자가 `reference_path`를 안 채운 데이터 문제"라고
적었다. 사용자가 바로 잡아 줬다 — **콘솔에 다 들어 있다.**

진짜 원인은 툴 계약이었다. `shared/manual_search.py`는 결과마다
`guide_location`/`guide_document`를 채워 넣는데, **Manual MCP `search_manual`의 docstring
`Returns:`에는 그 이름이 한 번도 없었다.** 지시문은 "`guide_location` 값을 그대로 옮겨
적으라"고 하는데, 모델이 보는 툴 스키마에는 그런 필드가 있다는 말이 없다.
지시문과 툴 계약이 서로 다른 세계를 얘기하고 있었다.

→ docstring에 세 필드를 명시했다(위치는 **한 글자도 바꾸지 말 것**까지). 회귀 테스트
`test_manual_tool_docstring_declares_reference_fields`.

**교훈**: 모델이 어떤 필드를 안 쓰면 지시문부터 고치지 말고 **툴이 그 필드를 선언하고 있는지**
먼저 본다. 모델에게 툴의 출력은 docstring이 말해 주는 것이 전부다.

### (2) 매뉴얼 검색을 모델의 선택지에서 뺐다 — 선검색(prefetch)

"매뉴얼을 먼저 검색하라"를 #149·#153·#154 세 번 지시문에 썼다. 그래도 모델이 자기가 안다고
판단하면 툴을 건너뛰었다(ETX 클라이언트 사용법을 지어낸 건이 그것이다).
**부를지 말지를 모델에게 맡기지 않는다.**

`agent_server/main.py`의 `_manual_context()`가 **매 질문마다** 매뉴얼을 검색해서
`# 이번 질문에 대한 매뉴얼 검색 결과` 블록으로 시스템 지시문 뒤에 붙인다. 세 엔드포인트
(`/v1/chat/completions`, `/v1/agent/query`, `/v1/voc/query`) 전부.

- Manual MCP를 거치지 않고 **같은 함수**(`search_manual_chunks`)를 직접 부른다. 검색 경로가
  하나뿐이라 콘솔 검색 테스트·MCP·선검색의 결과가 갈릴 수 없다.
- 블록에 `문서 위치`(= `guide_location`)와 `문서 이름`을 근거마다 실어 준다. 이제 모델은
  위치를 **못 볼 수가 없다** — (1)의 문제까지 이중으로 막힌다.
- 0건이면 "검색 결과가 없습니다. 지어내지 말고 운영팀 문의로 안내하세요"를 넣는다.
- 툴 `search_manual`은 그대로 남는다. 첫 결과로 부족하면 **다른 표현으로** 다시 찾으라고
  지시문에 적었다(같은 검색을 또 부르지 말라는 말도 함께).
- 실패해도 답변은 계속된다(매뉴얼 DB·임베딩이 죽어도 서비스는 떠 있어야 한다).
- 끄는 스위치: `manual_prefetch`(기본 켬), 개수 `manual_prefetch_top_k`(기본 3).

비용은 임베딩 1회 + 리랭킹 1회 ≈ 0.2~0.4초다. 대신 모델이 툴을 부르러 한 바퀴 더 도는
LLM 왕복(2~5초)이 대부분 사라지므로 **평균적으로는 오히려 빨라진다.**

### (3) 경고를 붙이는 대신 **답변을 갈아 끼운다** — `_Grounding` → `_AnswerGuard`

#154에서 근거 없는 값을 찾으면 답변 뒤에 "⚠️ 그대로 믿지 마세요"를 붙였다. 사용자가 거절했고,
그 지적이 옳다. **틀린 IP를 보여주면서 믿지 말라고 하는 것은 답이 아니다.** 사용자는 화면에
보이는 IP를 복사해서 접속해 본다.

`_AnswerGuard.review(answer)`는 근거가 없으면 그 본문을 **통째로** 이것으로 바꾼다.

```
문의하신 내용은 매뉴얼과 과거 사례에서 확인되지 않았습니다.
정확하지 않은 정보를 드리지 않기 위해 답변을 드리지 않습니다. 운영팀에 문의해 주세요.

접수 경로: <설정 voc_intake_guide>
```

바꾸려면 **본문이 아직 화면에 안 나가 있어야 한다.** 이미 흘려보낸 글자는 거둘 수 없다.
그래서 검사가 켜져 있으면(`_AnswerGuard.hold`) 스트리밍에서 본문 델타를 흘리지 않고 끝까지
모았다가 한 번에 내보낸다. **진행 줄(도구 실행 표시)은 그대로 흐르므로** 사용자는 기다리는
동안 무슨 일이 벌어지는지 계속 본다. 한 번에 나오는 게 싫으면 `answer_grounding_check`를
끄면 되지만, 그러면 지어낸 값이 그대로 나간다.

선검색 결과는 `seed_manual()`로 근거에 등록한다 — 모델이 툴을 안 불러도 매뉴얼을 본 것과
같기 때문이다. **단 0건이면 등록하지 않는다.** 그 상태에서 문서를 안내하면 지어낸 것이다.

### 지시문에서 고친 것

- "모르면 모른다" → **"확인되지 않으면 운영팀 문의로 안내한다"** (갈 곳을 준다).
- **"경고를 붙여 빠져나가지 않습니다"** 를 절대 규칙에 넣었다. "정확하지 않을 수 있습니다",
  "참고만 하세요", "그대로 믿지 마세요"를 붙인 추측은 그냥 추측이다.
- 도구 선택 절 맨 위에 **"매뉴얼 근거는 이미 와 있습니다"** — 선검색 블록을 먼저 읽고,
  같은 것을 다시 검색하지 말라고 적었다.

6,437자 / 4,020토큰(전체 컨텍스트의 12%). 선검색 블록이 근거 3건 기준 2,000~2,800토큰
더 붙는다. `ContextWindowExceededError`가 나면 `manual_prefetch_top_k`를 2로 내린다.

---

## #156. VOC도 선검색하고, 이어지는 질문의 문맥을 검색에 넣는다

#155를 배포하기 전에 같은 증상이 다시 보고됐다. Errors 원문:

> 1. 사용자 문의가 들어오면 **무조건!!! 제발!!!!** manual_mcp, voc_mcp 로 rag 한 후에 답변 생성.
> 2. (운영팀 안내 후 이어서) `~~~ 접속이 안되면 슈퍼컴 접속 못 하는거 아니야?`
>    → 다른 접속 가이드를 알려줘야 하는데 안 알려줌(manual db에 있음).
>    `login server 접속이 갑자기 안됩니다…` → **바로 운영팀 확인이 필요하다**고 하는데,
>    manual_db 기반으로 **우선 사용자가 확인해야 할 사항들 먼저 쭉 가이드를 하고, 그 후에**
>    운영팀한테 문의하라고 해야지.
> 3. `~~ 클러스터`는 슈퍼컴 인프라가 아니니까 다른 운영팀에 문의하라고 하는데 전혀 아님.
>    **voc_db에 있는 question 그대로 물어봤는데** answer가 이상함.

네 가지를 고쳤다.

### (1) VOC도 선검색한다 — #155에서 매뉴얼만 한 것이 반쪽이었다

"voc_db에 있는 질문을 그대로 물었는데 엉뚱한 답"은 **VOC 검색이 아예 안 돌았다**는 뜻이다.
매뉴얼과 똑같은 이유였다 — 모델이 툴을 부를지 말지 정한다.

`_manual_context` → **`_rag_context`**. 매뉴얼과 VOC를 `asyncio.gather`로 **동시에** 검색해
두 블록을 프롬프트에 넣는다(합이 아니라 둘 중 느린 쪽이 지연이다). 세 엔드포인트 전부.

그러려면 VOC 검색 함수가 필요한데, 그게 VOC MCP 안에만 있었다. `shared/voc_search.py`로
뺐다(`manual_search.py`와 같은 자리). MCP는 이제 그것을 부르는 껍데기 40줄이다.
**부르는 쪽이 둘이어도 검색 로직은 하나**여야 결과가 갈리지 않는다 —
회귀 테스트 `test_voc_search_has_one_shared_implementation`이 MCP 안에 SQL이 복제되는 것을 막는다.

### (2) 이어지는 질문은 그 문장만으로 검색이 안 된다 — 직전 사용자 발화를 붙인다

`그러면 슈퍼컴 접속 못 하는거 아니야?` — 여기에 검색할 명사가 뭐가 있나. 없다.
벡터든 키워드든 3-gram이든 이 문장으로는 접속 가이드를 못 찾는다. 사용자가 본 그대로다.

`_retrieval_query()`가 **직전 사용자 발화**를 앞에 붙여 검색한다. 그러면
`login server 접속이 갑자기 안됩니다 / 그러면 슈퍼컴 접속 못 하는거 아니야?`가 되어
같은 문서를 찾아낸다.

**어시스턴트 답변은 붙이지 않는다.** 앞 턴에서 지어낸 내용이 질의에 섞이면 그 방향으로
검색이 끌려간다 — 틀린 답 → 틀린 근거 → 틀린 답의 고리가 생긴다.

### (3) 운영팀 문의는 **맨 마지막** — #155의 폴백이 지나쳤다

#155에서 근거에 없는 값이 하나라도 있으면 답변을 **통째로** 버리고 운영팀 문의로 바꿨다.
그게 이번 지적과 정면으로 부딪힌다. 매뉴얼에서 확인된 점검 목록 다섯 줄 중 한 줄에 지어낸
IP가 있다고 해서 나머지 네 줄까지 버리면, 사용자는 **스스로 할 수 있는 일을 못 하게 된다.**
...
|---|---|
| 지어낸 값 없음 | 그대로 |
| 일부 줄에 지어낸 값 | **그 줄만** 덜어내고 나머지 + "확인되지 않은 내용은 제외했습니다. 더 필요하시면 운영팀에…" |
| 덜어내고 나니 껍데기(실질 20자 미만) | 통째로 운영팀 문의 |

지어낸 값이 사용자 화면에 **보이지 않는다**는 원칙은 그대로다. 버리는 단위만 답변 전체에서
줄로 줄였다.

지시문도 같이 고쳤다. 절대 규칙 3번이 "확인되지 않으면 운영팀 문의"였는데
**"운영팀 문의는 맨 마지막입니다 — 사용자가 직접 확인하거나 해 볼 수 있는 것을 먼저 전부
안내하고, 그걸로도 안 되면 그때"** 로 바꿨다. VOC의 `operator` 사례 규칙도 같다 — 남이 해 준
조치를 시키지는 않되, **매뉴얼에 사용자가 해 볼 것이 있으면 그것부터.**

### (4) 무엇이 우리 소관인지 모델이 정하지 않는다

`~~ 클러스터는 슈퍼컴 인프라가 아니니까 다른 운영팀에` — 이건 지어낸 것이다. 모델은 그 클러스터가
우리 것인지 알 방법이 없다. 낯선 이름을 보고 범위 밖이라고 판정하는 길을 지시문에서 끊었다:
**"어떤 시스템·클러스터가 우리 소관인지 스스로 판단하지 않습니다. 검색해서 나오면 그 내용으로
답하고, 안 나오면 확인되지 않는다고만 답합니다."**

### 설정

`manual_prefetch` → **`rag_prefetch`**(매뉴얼+VOC 한 스위치), `voc_prefetch_top_k`(기본 3) 추가.
지시문 6,977자 / 4,361토큰.

**교훈 두 개.** 하나 — 같은 원인을 고칠 때는 **형제도 같이 본다.** #155에서 "모델이 매뉴얼
툴을 안 부른다"를 고치면서 VOC도 똑같은 처지라는 것을 확인했어야 했다. 둘 — **안전장치가
품질을 깎지 않는지 본다.** #155의 전면 폐기는 지어내기는 막았지만 사용자가 할 수 있는 안내까지
같이 막았다. 안전장치는 나쁜 것만 정확히 덜어내야 한다.

---

## #157. 처리 주체를 키워드가 아니라 LLM이 판정한다

사용자 질문에서 시작했다 — "그러면 voc mcp가 user인지 operator인지 어떻게 파악하는 거야?"

답: **답변 텍스트에 특정 표현이 있는지 보는 키워드 매칭 하나뿐이었다.** `_OPERATOR_HINTS`
40여 개(`확인 결과`, `재기동`, `담당자가` …)에 하나라도 걸리면 `operator`, 아니면 `user`.

사용자: **"키워드 사용에 한계가 있더라고. 너가 잘 분류하도록 짜볼 수 있어?"** — 맞다.
표현은 무한하고, 목록은 늘려도 계속 뚫린다. #145에서 지시문에 대해 배운 것과 같은 실패다.

### 코드가 주석과 반대로 동작하고 있었다

주석에는 "애매하면 operator로 **보수적으로** 분류한다"라고 적혀 있었다. 실제로는
매칭이 안 되면 `user`로 떨어진다 — **정반대다.** 운영자가 조치한 건인데 답변이
"완료되었습니다"로만 끝나면 `user`가 되고, 에이전트는 남이 해 준 조치를 사용자에게 시킨다.
가장 위험한 방향의 오분류가 기본값이었다.

### 판정 기준을 하나로 좁혔다

에이전트가 실제로 내려야 하는 결정은 하나다 — **"이 방법을 안내해도 되는가?"**
그러면 판정할 것도 하나로 좁혀진다.

> **답변에 적힌 조치를, 문의한 그 사용자가 자기 계정 권한으로 그대로 다시 할 수 있는가?**

낱말이 아니라 **권한**을 묻는다. 답변이 어떤 표현을 쓰든 이 질문의 답은 같다.

### 적용한 분류 기법

| 기법 | 이유 |
|---|---|
| **기준(rubric) 기반** | 키워드 목록으로 돌아가지 않는다. 회귀 테스트가 목록이 프롬프트로 새어 들어오는 것을 막는다 |
| **`reason` → `label` 순서 강제** | 토큰은 왼쪽에서 오른쪽으로 생성된다. 라벨이 **자기가 방금 쓴 근거를 조건으로** 나온다. 뒤집으면 근거가 사후 정당화가 되고 정확도가 떨어진다 |
| **구조화 출력(guided decoding)** | `response_format: json_schema`로 문법 제약 디코딩. 스키마를 벗어난 토큰이 아예 생성되지 않아 파싱 실패·라벨 오타가 원천 소멸 |
| **기권 라벨 `unknown`** | 애매한 건을 찍지 않는다. "모르면 모른다"가 여기서도 같다 |
| **온도 0** | 같은 입력에 같은 라벨. 재분류가 의미를 가지려면 필요하다 |
| **id 에코 + 대조** | 배치 분류가 조용히 어긋나는 두 경우(없는 id를 지어냄 / 한 칸씩 밀려 답함)를 잡는다. 걸린 건은 **버리고 미분류로 남긴다** |
| **사고 블록 off** | 근거는 스키마의 `reason`으로 이미 받는다. Qwen3의 `<think>`까지 켜면 배치당 토큰이 몇 배가 된다 |

구조화 출력을 거부하는 서버(구버전 vLLM 등)에서는 자동으로 평문 JSON으로 내려가고,
그때만 파서가 `<think>` 블록·```json 펜스·앞뒤 설명 문장을 걷어낸다. **두 경로 다
가짜 LLM으로 태워서 확인했다.**

### 저장하고, 검색은 그것을 읽는다

`voc_records`에 `handled_by` / `handled_by_reason` / `handled_by_source` / `handled_by_at`
(마이그레이션 v8). 검색은 저장된 값을 쓰고, **아직 분류 전인 행에서만** 옛 키워드 추론으로
떨어진다. 그래서 백필을 돌리면 그 경로가 비워진다.

- 검색 세 갈래(키워드 전용 / 3축 RRF / 2축 RRF) **모두** 컬럼을 읽어야 한다 — 하나라도
  빠지면 그 경로에서만 조용히 키워드로 떨어진다. 회귀 테스트가 세 갈래를 센다
  (한 갈래를 지워 실제로 실패하는 것을 확인했다).
- 답변을 수정하면 판정을 `NULL`로 비운다. 안 그러면 옛 판정이 계속 쓰인다.
- `unknown`은 프롬프트에서 **운영자 건과 같이** 다룬다. 남이 해 준 조치를 사용자에게 시키는
  쪽이 반대 오류보다 나쁘다.

### 돌리는 방법

콘솔 VOC 탭에 카드가 생겼다(현황 + `먼저 30건만` / `미분류 전부`, 2초 폴링으로 진행 표시).
서버 터미널에서는 `bash scripts/classify-voc.sh 30` → `bash scripts/classify-voc.sh`.
`show` 모드로 판정 근거 표본만 볼 수도 있다. 이미 판정된 행은 건드리지 않으므로 몇 번을
돌려도 되고, 중간에 끊어도 그때까지는 저장된다.

**교훈**: 사용자가 "이건 어떻게 동작해?"라고 물으면, 대답하면서 그 코드가 **주석대로 동작하는지**
같이 본다. 이번 키워드 분류기는 주석과 정반대로 동작하고 있었고, 아무도 그것을 몰랐다.
