...
**(3) 문서에 없는 CUDA 아키텍처 플래그를 계속 붙이는 문제**(`-arch=compute_90` 등).
→ 지시문 (A)에 **금지 목록을 구체적으로** 못 박고(아키텍처 플래그·모듈 로드 순서·배치 지시자·
환경변수 예시·권장 파라미터), "답을 다 쓴 뒤 문장 하나하나가 조회 결과에 있는지 확인하고
없는 문장은 지운다"를 추가했다.
