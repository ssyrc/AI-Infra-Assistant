...
```bash
# .env (예시 — 실제 사내 인덱스 repo로 교체)
PIP_INDEX_URL=http://repository.samsungds.net/repository/<PyPI-INDEX-PROXY>/simple
PIP_TRUSTED_HOST=repository.samsungds.net
BUILD_PROXY=http://202.20.187.241:3128
```

⚠️ **`PIP_INDEX_URL`은 반드시 'simple 인덱스'를 가리켜야 한다** — `pypi.org/simple`을 프록시하는
Nexus repo. `files.pythonhosted.org`(패키지 파일 호스트) 프록시를 인덱스로 쓰면 pip가
`json.decoder.JSONDecodeError: Expecting value: line 1 column 1` 로 실패한다. 확인:

```bash
# 200 + 패키지 목록(HTML/JSON)이 나와야 정상. 404/빈 응답이면 인덱스 URL이 틀린 것.
curl -sI --proxy $BUILD_PROXY $PIP_INDEX_URL/pip/
```
...
curl -s http://localhost:8000/v1/models ; echo
```
...
```bash
curl -s http://localhost:8000/v1/agent/query -H 'Content-Type: application/json' -d '{
...
```bash
curl -s http://localhost:8000/v1/voc/query -H 'Content-Type: application/json' -d '{
  "voc_info":{
...
스트리밍:
```bash
curl -N http://localhost:8000/v1/voc/query -H 'Content-Type: application/json' -d '{
