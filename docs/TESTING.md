...
<REGISTRY_DOCKERHUB>/python:3.11-slim-bullseye        # MCP/agent/admin/mock 빌드 베이스
...
이미지 빌드 중 `pip install`이 사내 PyPI 미러/프록시를 쓰도록 `.env`에 설정한다(값은 사내 pip.conf와 동일하게):

```bash
# .env (사내 pip.conf 값 그대로: index-url / proxy / trusted-host)
...
# 사내 pip.conf에 no_proxy가 없으면(미러도 프록시를 통해 접근) localhost만 둔다.
NO_PROXY=localhost,127.0.0.1
```

### ⚠️ 빌드 중 `JSONDecodeError: Expecting value: line 1 column 1` — 원인과 해결(이미 반영됨)

이건 **프록시 문제가 아니다.** 내장 **최신 pip(24.x)** 는 **JSON Simple API(PEP 691)** 로
인덱스를 조회하는데, 사내 미러가 이를 지원하지 않아 pip가 응답을 `json.loads()` 하다 깨진다.
사내 호스트의 pip가 동작하는 이유는 그 pip가 **HTML Simple API만 쓰는 옛 버전(<22.3)** 이기 때문이다.
(베이스 이미지는 `python:3.11-slim-bullseye` — 옛 pip는 3.12에서 `pkgutil.ImpImporter` 에러로 못 돎. 자세한 건 `docs/CLAUDE-GUIDE.md` 3장.)
