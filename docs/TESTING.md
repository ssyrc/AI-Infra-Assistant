...
이건 **프록시 문제가 아니다.** `python:3.12-slim`이 내장한 **최신 pip(24.x)** 는
**JSON Simple API(PEP 691)** 로 인덱스를 조회하는데, 사내 미러가 이를 지원하지 않아 pip가
응답을 `json.loads()` 하다 깨진다. 사내 호스트의 pip가 동작하는 이유는 그 pip가 **HTML Simple
API만 쓰는 옛 버전(<22.3)** 이기 때문이다.
