...
from retrieval import ts_or_query, expand_query, has_trgm, mmr_dedup, trgm_min_similarity
...
async def _candidates(pool, query: str, candidate_k: int) -> tuple[str, list[dict]]:
    ...
    try:
        ...
        print(f"[manual-search] 임베딩 실패, 키워드 검색으로 fallback: {type(e).__name__}: {e}")
...
async def search_manual_chunks(query: str, top_k: int = 5, *,
                               with_neighbors: bool = True) -> tuple[str, list[dict]]:
    ...
    mode, candidates = await _candidates(pool, query, candidate_k)
    if not candidates:
        return mode, []
