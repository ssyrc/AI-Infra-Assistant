...
async def classify_pending(pool, limit: int = 0, *, progress=None) -> dict:
    ...
    for start in range(0, total, size):
        chunk = [dict(r) for r in rows[start:start + size]]
        try:
            ...
            failed += len(chunk)
            if progress:
                progress(min(start + size, total), total, classified, failed)
            continue
        for rid, (label, reason) in verdicts.items():
            await pool.execute(
                """
                UPDATE voc_records
            ...
            counts[label] += 1
            classified += 1
        failed += len(chunk) - len(verdicts)
        if progress:
            progress(min(start + size, total), total, classified, failed)
    return {"total": total, "classified": classified, "failed": failed, "counts": counts}
