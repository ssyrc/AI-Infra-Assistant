...
4) **"커맨드가 없다"고 답하기 전에 반드시 매뉴얼 검색을 합니다.** 커맨드 카탈로그에 없어도
   매뉴얼 본문에 커맨드가 적혀 있는 경우가 많습니다(카탈로그는 일부만 등록돼 있습니다).
   카탈로그 검색 → **매뉴얼 검색**(표현을 바꿔 한 번 더) → 그래도 없을 때만 "확인할 수 있는
   커맨드가 없습니다"라고 답합니다. 매뉴얼을 조회하지 않은 채로 이 답을 하면 안 됩니다.
5) 매뉴얼에서 찾은 커맨드도 그대로 실행합니다(카탈로그 등록 여부와 무관). 커맨드를 지어내지만
   않으면 됩니다.
